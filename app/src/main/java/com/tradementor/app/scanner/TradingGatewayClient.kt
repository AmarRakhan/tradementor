...
data class CloudDcaDeal(
    val symbol: String = "",
    val strategyId: String = "strategy_3",
    val shortDirection: Boolean = false,
    val initialEntryPrice: Double = 0.0,
    val lastOrderPrice: Double = 0.0,
    val safetyOrdersCompleted: Int = 0
)

private data class CloudDcaDealsResponse(val deals: List<CloudDcaDeal> = emptyList())

data class TradingGatewayCloseResult(
    val closed: Boolean = false,
    val symbol: String = "",
    val percentage: Int = 100,
    val closedSize: Double = 0.0,
    val remainingSizeEstimate: Double = 0.0,
...
data class TradingCycleStatus(
    ...
    val startPortfolioValue: Double = 0.0,
    val currentPortfolioValue: Double = 0.0,
    val targetPercentage: Double = 10.0,
    val targetPortfolioValue: Double = 0.0,
    val growthPercentage: Double = 0.0,
    val remainingUsd: Double = 0.0,
    val progressPercentage: Double = 0.0,
    val targetReached: Boolean = false,
    val originalStartPortfolioValue: Double = 0.0,
    val externalCashFlowUsd: Double = 0.0,
    val cashFlowDataComplete: Boolean = false,
    val startedAtEpochMs: Long = 0L,
    val closed: List<Map<String, Any>> = emptyList(),
    val failed: List<Map<String, String>> = emptyList()
)
...
object LocalTradingGatewayStore {
    ...
    val DEFAULT_URL = BuildConfig.CLOUD_API_URL.ifBlank {
...
class TradingGatewayClient {
    ...
    suspend fun dcaDeals(baseUrl: String): List<CloudDcaDeal> = withContext(Dispatchers.IO) {
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/dca/deals").get())
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
            check(response.isSuccessful) { "DCA-status kon niet centraal worden geladen (${response.code})" }
            Gson().fromJson(body, CloudDcaDealsResponse::class.java).deals
        }
    }
    ...
    suspend fun executeOneTest(
        ...
        strategyId: String,
        takeProfitEnabled: Boolean = true,
        trailingTakeProfitEnabled: Boolean = false,
        trailingDeviationPercentage: Double = 0.5,
        stopLossEnabled: Boolean = true,
        topUniverseSize: Int = 50
        ...
        val payload = mapOf(
            ...
            "take_profit_enabled" to takeProfitEnabled,
    ...
    suspend fun executeAddOn(
        ...
        intent: HyperliquidOrderIntent,
        profitPercentage: Double,
        strategyId: String = "strategy_1",
        safetyOrderIndex: Int = 1,
        maxSafetyOrders: Int = 1,
        maxDealValueUsd: Double = 1_000_000.0,
        maxAdversePercentage: Double = 8.0,
        takeProfitEnabled: Boolean = true,
        trailingTakeProfitEnabled: Boolean = false,
        trailingDeviationPercentage: Double = 0.5,
        stopLossEnabled: Boolean = true
        ...
        val payload = mapOf(
            ...
            "profit_percentage" to profitPercentage,
            "strategy_id" to strategyId,
            "safety_order_index" to safetyOrderIndex,
            "max_safety_orders" to maxSafetyOrders,
            "max_deal_value_usd" to maxDealValueUsd,
            "max_adverse_percentage" to maxAdversePercentage,
            "take_profit_enabled" to takeProfitEnabled,
            "trailing_take_profit_enabled" to trailingTakeProfitEnabled,
            "trailing_deviation_percentage" to trailingDeviationPercentage,
            "stop_loss_enabled" to stopLossEnabled,
            "idempotency_key" to "dca-${intent.id}-${intent.symbol.uppercase()}-$safetyOrderIndex"
        )
        val request = Request.Builder()
    ...
    suspend fun startTradingCycle(baseUrl: String, targetPercentage: Double, portfolioValue: Double): TradingCycleStatus = withContext(Dispatchers.IO) {
        val payload = Gson().toJson(mapOf("target_percentage" to targetPercentage, "portfolio_value" to portfolioValue))
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/trading/cycle/start")
            .post(payload.toRequestBody("application/json".toMediaType())))
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
            check(response.isSuccessful) { runCatching { Gson().fromJson(body, Map::class.java)["detail"].toString() }.getOrDefault("Handelscyclus kon niet starten") }
            Gson().fromJson(body, TradingCycleStatus::class.java)
        }
    }

    suspend fun updateTradingCycleTarget(baseUrl: String, targetPercentage: Double): TradingCycleStatus = withContext(Dispatchers.IO) {
        val payload = Gson().toJson(mapOf("target_percentage" to targetPercentage))
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/trading/cycle/target")
            .put(payload.toRequestBody("application/json".toMediaType())))
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
            check(response.isSuccessful) { runCatching { Gson().fromJson(body, Map::class.java)["detail"].toString() }.getOrDefault("Portfoliodoel kon niet worden bijgewerkt") }
            Gson().fromJson(body, TradingCycleStatus::class.java)
        }
    }

    suspend fun tradingCycleStatus(baseUrl: String, portfolioValue: Double): TradingCycleStatus = withContext(Dispatchers.IO) {
        val payload = Gson().toJson(mapOf("portfolio_value" to portfolioValue))
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/trading/cycle/status")
            .post(payload.toRequestBody("application/json".toMediaType())))
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
            check(response.isSuccessful) { runCatching { Gson().fromJson(body, Map::class.java)["detail"].toString() }.getOrDefault("Handelscyclus kon niet worden opgehaald") }
            Gson().fromJson(body, TradingCycleStatus::class.java)
        }
    }

    suspend fun evaluateTradingCycle(baseUrl: String, portfolioValue: Double): TradingCycleStatus = withContext(Dispatchers.IO) {
        val payload = Gson().toJson(mapOf("portfolio_value" to portfolioValue))
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/trading/cycle/evaluate")
            .post(payload.toRequestBody("application/json".toMediaType())))
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
            check(response.isSuccessful) { runCatching { Gson().fromJson(body, Map::class.java)["detail"].toString() }.getOrDefault("Portfoliodoel kon niet worden gecontroleerd") }
            Gson().fromJson(body, TradingCycleStatus::class.java)
        }
    }
    ...
    suspend fun closePosition(baseUrl: String, token: String, symbol: String, percentage: Int = 100): TradingGatewayCloseResult = withContext(Dispatchers.IO) {
        ...
        val request = authenticated(Request.Builder().url("$baseUrl/v1/me/positions/$encoded/close")
            .post(Gson().toJson(mapOf("confirm" to true, "percentage" to percentage)).toRequestBody("application/json".toMediaType())))
        http.newCall(request).execute().use { response ->
            val body = response.body?.string().orEmpty()
