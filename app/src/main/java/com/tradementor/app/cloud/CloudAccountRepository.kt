...
object CloudAccountRepository {
    ...
    /** Restores the already linked public wallet after reinstall or WalletConnect downtime. */
    suspend fun linkedWallet(): String = withContext(Dispatchers.IO) {
        ...
        val request = Request.Builder()
            .url("${BuildConfig.CLOUD_API_URL.trimEnd('/')}/v1/me/wallet")
            .header("Authorization", "Bearer $token")
            .get()
            .build()
        httpClient.newCall(request).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                val detail = runCatching { Gson().fromJson(body, Map::class.java)["detail"]?.toString() }.getOrNull()
                error(detail ?: "Cloudwallet kon niet worden hersteld (${response.code}).")
            }
            Gson().fromJson(body, Map::class.java)["address"]?.toString().orEmpty()
                .takeIf { it.startsWith("0x") && it.length == 42 }
                ?: error("Cloudaccount bevat geen geldig walletadres.")
        }
    }
