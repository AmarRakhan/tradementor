    ...
    LazyColumn(
        ...
        item {
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                ...
                TextButton(onClick = { showPeriodDialog = true }) { Text("Periode · ${selectedPeriod.title}", color = Color(0xFF9DB4FF), fontSize = 9.sp) }
                TextButton(onClick = { confirmClearAll = true }) { Text("Alles verwijderen", color = HistoryRed, fontSize = 9.sp) }
            }
    ...
    tradeToDelete?.let { trade ->
        AlertDialog(
            ...
            confirmButton = { TextButton(onClick = {
                ...
                tradeToDelete = null
            }) { Text("Verwijderen", color = HistoryRed) } },
            dismissButton = { TextButton(onClick = { tradeToDelete = null }) { Text("Annuleren") } }
        )
    }
    if (confirmClearAll) {
        AlertDialog(
            ...
            }) { Text("Alles verwijderen", color = HistoryRed) } },
            dismissButton = { TextButton(onClick = { confirmClearAll = false }) { Text("Annuleren") } }
        )
    }
    if (showPeriodDialog) {
        AlertDialog(
            ...
            confirmButton = { TextButton(onClick = { showPeriodDialog = false }) { Text("Sluiten") } }
        )
    }
}
