...
fun AppSettingsScreen(
    onBack: (() -> Unit)? = null,
    onOpenFeedback: () -> Unit = {},
    backtestVisible: Boolean = true,
    onBacktestVisibleChange: (Boolean) -> Unit = {},
    marketsVisible: Boolean = true,
    onMarketsVisibleChange: (Boolean) -> Unit = {},
    signalsVisible: Boolean = true,
    onSignalsVisibleChange: (Boolean) -> Unit = {},
    ...
    PullToRefreshBox(
        ...
        Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(18.dp)) {
            if (onBack != null) {
                Button(
                    onClick = onBack,
                    colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF16274A)),
                    modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp)
                ) {
                    Text("← Terug naar Wallet", color = Color.White, fontWeight = FontWeight.Bold)
                }
            }
            ...
            SettingsCard(tr(language, "main_tabs")) {
                Row(modifier = Modifier.fillMaxWidth().padding(15.dp)) {
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Backtest tonen", color = Color.White, fontWeight = FontWeight.SemiBold)
                        Text("Verberg of toon het losse Backtest-tabblad.", color = Color(0xFF8C92A3), fontSize = 11.sp)
                    }
                    Switch(checked = backtestVisible, onCheckedChange = onBacktestVisibleChange)
                }
                HorizontalDivider(color = Color(0xFF232A38))
                Row(modifier = Modifier.fillMaxWidth().padding(15.dp)) {
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Markets tonen", color = Color.White, fontWeight = FontWeight.SemiBold)
                        Text("Zet het losse Markets-tabblad aan of uit. Signals blijft alle marktdata gebruiken.", color = Color(0xFF8C92A3), fontSize = 11.sp)
                    }
                    Switch(checked = marketsVisible, onCheckedChange = onMarketsVisibleChange)
                }
                HorizontalDivider(color = Color(0xFF232A38))
                Row(modifier = Modifier.fillMaxWidth().padding(15.dp)) {
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Signals tonen", color = Color.White, fontWeight = FontWeight.SemiBold)
                        Text("Verberg of toon het scanner- en signalentabblad.", color = Color(0xFF8C92A3), fontSize = 11.sp)
                    }
                    Switch(checked = signalsVisible, onCheckedChange = onSignalsVisibleChange)
                }
            }
            Spacer(Modifier.height(12.dp))
                ...
                Column(modifier = Modifier.padding(15.dp)) {
                    ...
                    if (!lockEnabled) {
                        ...
                        Button(
                            ...
                            colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2F68FF)),
                            modifier = Modifier.fillMaxWidth()
                        ) { Text("Appvergrendeling instellen") }
                    } else {
                        ...
                        Spacer(Modifier.height(12.dp))
                        Button(onClick = { showDisableDialog = true }, modifier = Modifier.fillMaxWidth()) {
                            Text("Appvergrendeling uitschakelen")
                        }
                    }
            ...
            SettingsCard(tr(language, "scanner_notifications")) {
                ...
                if (BuildConfig.APPLICATION_ID.endsWith(".test")) {
                    ...
                    Button(
                        ...
                        colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF0B7A55)),
                        modifier = Modifier.fillMaxWidth().padding(15.dp)
                    ) {
                        Text("Test gekozen winstgeluid")
                    }
                }
            }
            Spacer(Modifier.height(12.dp))
    ...
    if (showDisableDialog) {
        AlertDialog(
            ...
            dismissButton = {
                TextButton(onClick = { showDisableDialog = false; currentPin = "" }) { Text("Annuleren") }
            }
        )
    }
}
