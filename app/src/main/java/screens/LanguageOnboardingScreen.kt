...
fun LanguageOnboardingScreen(onConfirmed: (AppLanguage) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().background(Color(0xFF05070B)).padding(horizontal = 20.dp, vertical = 28.dp)) {
        ...
        Button(onClick = { onConfirmed(selected) }, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2F68FF)), modifier = Modifier.fillMaxWidth().height(54.dp)) {
            Text(if (selected == AppLanguage.Dutch) "Doorgaan" else "Continue", fontWeight = FontWeight.Bold)
        }
    }
}
