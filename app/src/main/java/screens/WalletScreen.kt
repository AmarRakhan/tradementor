...
private val WalletGreen = Color(0xFF2AE29B)
private val WalletBlue = Color(0xFF597DFF)
private val warningYellow = Color(0xFFFFC857)
private val successRed = Color(0xFFFF496A)
...
private fun WalletConnectionFlipCard(
    ...
    val theme = LocalTradeMentorColors.current
    Surface(
        color = if (connected) theme.surface.copy(alpha = 0.72f) else theme.surfaceRaised.copy(alpha = 0.72f),
        ...
        if (rotation <= 90f) {
            Row(
                ...
                Text(if (connected) "✓" else "•", color = if (connected) WalletGreen else warningYellow, fontSize = 15.sp)
                Column(Modifier.weight(1f).padding(start = 10.dp)) {
                    Text(title, color = Color.White, fontSize = 14.sp, fontWeight = FontWeight.Bold)
                    Text(summary, color = WalletMuted, fontSize = 10.sp, maxLines = 1)
                }
                Text(if (connected) "✓" else "!", color = if (connected) WalletGreen else warningYellow, fontSize = 18.sp, fontWeight = FontWeight.Black)
            ...
            Column(Modifier.fillMaxWidth().graphicsLayer { rotationY = 180f }.padding(15.dp)) {
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text(if (connected) "✓ VERBONDEN" else "• ACTIE NODIG", color = if (connected) WalletGreen else warningYellow, fontSize = 9.sp, fontWeight = FontWeight.Black)
                }
                Spacer(Modifier.height(10.dp))
                details()
                TradeMentorTextButton(label = "Terug naar overzicht", onClick = onFlip, modifier = Modifier.fillMaxWidth())
            }
        }
    }
}
...
fun WalletScreen(onOpenWallet: () -> Unit, onOpenSettings: () -> Unit = {}) {
    ...
    LazyColumn(
        ...
        item {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                TradeMentorGhostButton(label = "Appinstellingen", onClick = onOpenSettings)
            }
        }
            ...
            item {
                WalletConnectionFlipCard(
                    ...
                    Column(Modifier.fillMaxWidth().padding(16.dp)) {
                        Text("AUTOMATISCH HANDELEN · VERSIE 1.65", color = if (apiWalletConfigured) WalletGreen else warningYellow, fontSize = 10.sp, fontWeight = FontWeight.ExtraBold)
                        ...
                        TradeMentorPrimaryButton(
                            label = if (gatewayHealth?.agentWalletConfigured == true) "Cloudkoppeling actief" else if (apiWalletConfigured) "Mainnet-koppeling vernieuwen" else "Veilig met MetaMask koppelen",
                            enabled = !approvalRunning && address.isNotBlank() && gatewayHealth?.agentWalletConfigured != true,
                            onClick = {
                                approvalRunning = true
                                approvalMessage = "MetaMask openen voor Mainnet-machtiging…"
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            TradeMentorPrimaryButton(
                                label = if (manualMode) "Annuleren" else "Handmatig koppelen",
                                enabled = gatewayHealth?.agentWalletConfigured != true,
                                onClick = { manualMode = !manualMode },
                                modifier = Modifier.weight(1f)
                            )
                            if (apiWalletConfigured && gatewayHealth?.agentWalletConfigured != true) {
                                TradeMentorTextButton(
                                    label = "Verwijderen",
                                    onClick = { ApiWalletVault.clear(context); apiWalletConfigured = false },
                                    modifier = Modifier.weight(1f)
                                )
                            }
                        }
                        if (manualMode) {
                            ...
                            Spacer(Modifier.height(10.dp))
                            TradeMentorPrimaryButton(
                                label = if (approvalRunning) "Verwerken…" else "Veilig opslaan in cloud",
                                enabled = !approvalRunning && apiKeyInput.isNotBlank(),
                                onClick = {
                                    approvalRunning = true
                                    approvalMessage = "API-wallet op Mainnet en in jouw persoonlijke cloud controleren…"
                                    scope.launch {
                                        runCatching {
                        ...
                        approvalMessage?.let { Text(it, color = if (apiWalletConfigured) WalletGreen else warningYellow, fontSize = 10.sp, modifier = Modifier.padding(top = 8.dp)) }
                    }
                }
            }
            item {
                ...
                WalletConnectionFlipCard(
                    ...
                    Column(Modifier.fillMaxWidth().padding(16.dp)) {
                        Text("TRADEMENTOR CLOUD", color = if (connected) WalletGreen else warningYellow, fontSize = 10.sp, fontWeight = FontWeight.ExtraBold)
                        ...
                        Surface(color = WalletCard, shape = RoundedCornerShape(17.dp)) {
                            Column(Modifier.padding(14.dp)) {
                                Row(Modifier.fillMaxWidth()) {
                                    ...
                                    Text(if (short) "SHORT" else "LONG", color = if (short) successRed else WalletGreen, fontWeight = FontWeight.Bold)
                                }
                                Spacer(Modifier.height(8.dp))
                                ...
                                WalletValueRow("Hefboom", "${position.leverage.value}Ã—  ·  ${position.leverage.type}")
                            }
                        }
                    }
                }
    ...
    accountWalletMismatch?.let {
        AlertDialog(
            ...
            confirmButton = {
                TradeMentorPrimaryButton(
                    label = "Opnieuw instellen en aanmelden",
                    onClick = {
            ...
            dismissButton = {
                TradeMentorTextButton(label = "Annuleren", onClick = { accountWalletMismatch = null })
            }
        )
    }
}
...
@Composable private fun ConnectWalletCard(onOpenWallet: () -> Unit) {
    Surface(color = WalletCard, shape = RoundedCornerShape(24.dp)) {
        Column(Modifier.fillMaxWidth().padding(22.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            TradeMentorPrimaryButton(label = "Wallet koppelen", onClick = onOpenWallet, modifier = Modifier.fillMaxWidth().height(52.dp))
...
private fun shortAddress(address: String) = if (address.length > 12) "${address.take(7)}â€¦${address.takeLast(5)}" else address
...
private fun price(number: String?): String = number?.toDoubleOrNull()?.let { NumberFormat.getNumberInstance(Locale.US).apply { maximumFractionDigits = 8 }.format(it) } ?: "â€”"
...
private fun pnlColor(number: String?) = if (value(number) >= 0) WalletGreen else successRed
private fun time(timestamp: Long) = SimpleDateFormat("dd MMM HH:mm", Locale("nl", "NL")).format(Date(timestamp))
// End of wallet formatting helpers.
