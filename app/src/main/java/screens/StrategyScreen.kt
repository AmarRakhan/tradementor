...
@Composable
fun StrategyScreen(purchasePreviewEnabled: Boolean = false) {
        ...
        Column(Modifier.fillMaxSize().padding(14.dp), verticalArrangement = Arrangement.SpaceBetween) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                ...
                if (purchasePreviewEnabled) {
                    Text(if (definition.id == "strategy_1") "CORE" else "PRO", color = if (definition.id == "strategy_1") StrategyGreen else Color(0xFFFFC857), fontSize = 7.sp, fontWeight = FontWeight.Black, modifier = Modifier.padding(end = 6.dp))
                }
...
private fun StrategyDetailPage(
    ...
    Column(Modifier.fillMaxSize().background(StrategyBg).verticalScroll(rememberScrollState()).padding(16.dp)) {
        Button(onClick = onBack, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF16274A)), modifier = Modifier.fillMaxWidth()) {
            Text("← Terug naar Strategy", color = Color.White, fontWeight = FontWeight.Bold)
        }
