...
fun LearningScreen(onBack: (() -> Unit)? = null) {
    ...
    Column(
        ...
        onBack?.let {
            Button(onClick = it, modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp)) { Text("← Terug naar Risk") }
        }
        ...
        Surface(color = LearnPanel, shape = RoundedCornerShape(16.dp), modifier = Modifier.fillMaxWidth()) {
            Column(modifier = Modifier.padding(15.dp)) {
                ...
                Button(onClick = { showProjectLog = true }, modifier = Modifier.fillMaxWidth()) {
                    Text("Open projectlogboek")
                }
            }
        }
...
private fun ProjectLogScreen(onBack: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize().background(LearnBg).statusBarsPadding().navigationBarsPadding().verticalScroll(rememberScrollState()).padding(18.dp)) {
        TextButton(onClick = onBack) { Text("‹ Launchpad") }
        ...
        Column(modifier = Modifier.padding(horizontal = 9.dp, vertical = 7.dp)) {
            ...
            if (expanded) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                    onDelete?.let { TextButton(onClick = it) { Text("Verwijderen", color = Color(0xFFFF4964)) } }
                    TextButton(onClick = { onChanged(task.copy(note = note.trim())); expanded = false }) { Text("Opslaan") }
                }
            }
        }
    }
