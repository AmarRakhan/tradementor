...
fun FeedbackScreen(onBack: () -> Unit) {
    ...
    LazyColumn(
        ...
        item {
            Button(onClick = onBack, modifier = Modifier.fillMaxWidth().padding(top = 12.dp), colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF16274A))) {
                Text("← Terug naar instellingen", fontWeight = FontWeight.Bold)
            }
            ...
            Surface(color = FeedbackCard, shape = RoundedCornerShape(18.dp)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Button(
                        ...
                        colors = ButtonDefaults.buttonColors(containerColor = FeedbackGreen),
                        modifier = Modifier.fillMaxWidth().padding(top = 12.dp)
                    ) { Text("Melding verzenden", color = Color(0xFF04120E), fontWeight = FontWeight.Black) }
                }
            }
        }
...
private fun FeedbackModeButton(label: String, selected: Boolean, onClick: () -> Unit) {
    if (selected) Button(onClick = onClick, colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF2F68FF))) { Text(label) }
