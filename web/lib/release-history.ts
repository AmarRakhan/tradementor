...
export const CURRENT_RELEASE: ReleaseHistoryEntry = {
  ...
  title: "PWA-opstart zonder herlaadlus",
  newItems: [
    "De geïnstalleerde Android/Samsung-PWA start nu zonder automatische service-worker reload tijdens de Firebase-sessiecontrole.",
    "De manifest-start-URL bevat direct de actuele hoofdversie, zodat de app niet eerst zichzelf hoeft door te sturen vanaf een incomplete PWA-URL.",
    "Een canonical-buildverschil mag per doelbuild nog maar één automatische replace uitvoeren; dezelfde target kan binnen dezelfde sessie geen herlaadlus veroorzaken.",
  ],
  problems: [
    "Bij openen of tussentijds vernieuwen kon de app meerdere keren wisselen tussen Beveiligde sessie controleren en een zwart scherm met spinner.",
    "Dit was zichtbaar als splash → zwart/spinner → splash → zwart/spinner tijdens één enkele PWA-start.",
  ],
  causes: [
    "De oude manifest-start-URL opende alleen met source=pwa; PwaRegistration zag het ontbrekende appVersion als afwijking en navigeerde direct opnieuw.",
    "De service worker gebruikt skipWaiting en clients.claim; de frontend koppelde daar ook nog controllerchange → window.location.reload() aan.",
    "Daar bovenop kon de canonical build-check nog een window.location.replace() uitvoeren wanneer tijdens een deployment een andere build werd gezien.",
  ],
  fixes: [
    "Ontbrekend appVersion veroorzaakt geen navigatie meer; alleen een expliciet verouderde versie-marker mag éénmalig worden gecorrigeerd.",
    "controllerchange veroorzaakt geen pagina-reload meer tijdens startup.",
    "De eerste canonical-buildcheck navigeert niet meer tijdens initial mount; latere visibility/60s-controles blijven bestaan.",
    "Automatische canonical refresh is per doelversie/build met sessionStorage begrensd tot één replace.",
  ],
  now: [
    "Een normale PWA-start blijft op één geladen appdocument terwijl Firebase en de cloudsessie herstellen.",
    "Een echte nieuwere build kan later nog steeds automatisch worden opgehaald, maar zonder meerdere reloads achter elkaar.",
  ],
  before: "Een service-worker update en buildcontrole konden tijdens de auth-splash meerdere pagina-navigaties achter elkaar starten.",
  after: "Service-worker takeover is stil; alleen een expliciete, begrensde canonical-buildwissel mag later één navigatie uitvoeren.",
  technicalDetails: [
    "Aangepast: PwaRegistration, manifest start_url, PWA-regressietests, versieadministratie.",
    "Geen trading-, order-, portfolio-, DCA-, hedge-, TP- of exchange-dataflow gewijzigd.",
  ],
