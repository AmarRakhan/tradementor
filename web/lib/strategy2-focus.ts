...
export function focusExposurePreview(input:FocusPreviewInput):FocusPreview{
 ...
 const safe=totalMaxOrderNotional<=budget+1e-9&&requiredMargin<=available+1e-9;const status=totalMaxOrderNotional>budget+1e-9?"budget overschreden":requiredMargin>available+1e-9?"onvoldoende beschikbare margin":"veilig";const totalDropToLastDcaPct=triggerDropsPct.length?triggerDropsPct[triggerDropsPct.length-1]:0;
 return {startNotional,dcaNotionals,totalMaxOrderNotional,requiredMargin,maxLeveragedExposure:totalMaxOrderNotional,portfolioMarginPct:input.equity>0?requiredMargin/input.equity:0,remainingFreeMargin:Math.max(0,available-requiredMargin),worstCaseAverageEntry:focusWeightedAverage(input.entryPrice,startNotional,triggerPrices,dcaNotionals),triggerPrices,triggerDropsPct,totalDropToLastDcaPct,focusBudgetNotional:budget,focusBudgetRemainingNotional:Math.max(0,budget-totalMaxOrderNotional),availableMargin:available,safe,status};
}
