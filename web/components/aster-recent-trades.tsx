...
function FocusV2CockpitPanel({value}:{value:FocusV2Cockpit}){
  ...
  return <section className={styles.cockpit} aria-label="Focus 2.0 live cockpit"><header><div><span>FOCUS 2.0 LIVE COCKPIT</span><strong>{value.nextAction||"Bescherming in balans"}</strong></div><em>{Math.round(Number(value.hedgeRatio||0)*100)}% HEDGE</em></header><div className={styles.cockpitGrid}><article><span>LONG</span><strong>{money(value.longNotional)} · {Math.round(Number(value.longLeverage||0))}x</strong><small>{amount(value.longQuantity)} · entry {amount(value.longEntry)} · PnL {money(value.longPnl,true)}</small></article><article><span>SHORT HEDGE</span><strong>{money(value.shortNotional)} · {Math.round(Number(value.shortLeverage||0))}x</strong><small>{amount(value.shortQuantity)} · entry {amount(value.shortEntry)} · PnL {money(value.shortPnl,true)}</small></article><article><span>NETTO / GROSS</span><strong>{money(value.netExposure)} / {money(value.grossExposure)}</strong><small>DCA {Math.round(Number(value.dcaCount||0))} · cycle {money(value.cyclePnl,true)}</small></article></div><div className={styles.cockpitRecovery}><div><span>Prijsherstel</span>{check(value.recoveryPriceMet)}</div><div><span>5m Bollinger-middle</span>{check(value.bollinger5mConfirmed)}</div><div><span>Portfolioherstel</span>{check(value.portfolioRecoveryMet)}</div><div><span>Volgende release</span><b className={value.shortReleaseReady?styles.cockpitOk:styles.cockpitWait}>{Math.round(Number(value.shortReleaseRatio||0)*100)}% SHORT</b></div></div><div className={styles.cockpitMetrics}><span>Volgende DCA <b>{Number(value.nextLongDcaPrice)>0?`${amount(value.nextLongDcaPrice)} · ${Number(value.nextLongDcaDistancePct||0).toFixed(2)}%`:"—"}</b></span><span>LONG groen <b>{amount(value.longBreakEvenPrice)}</b></span><span>SHORT release <b>{Number(value.nextShortReleasePrice)>0?amount(value.nextShortReleasePrice):"Wacht op bevestiging"}</b></span><span>Re-hedge <b>{value.rehedgeArmed?`${amount(value.rehedgePrice)} · GEWAPEND`:"Niet gewapend"}</b></span><span>Cycle equity <b>{money(value.cycleEquity)} / start {money(value.cycleStartEquity)}</b></span><span>DCA anchor <b>{amount(value.dcaAnchorPrice)}</b></span><span>Profit sinds harvest <b>{money(value.profitSinceHarvest,true)} / trigger {money(value.profitTriggerUsdt)}</b></span><span>Nog tot harvest <b>{money(value.profitRemainingUsdt)} · neem {money(value.profitHarvestUsdt)}</b></span><span>Laatste / totaal harvest <b>{money(value.lastHarvestProfit,true)} / {money(value.totalHarvestedProfit,true)}</b></span></div>{actions.length>0&&<div className={styles.cockpitActions}><header><strong>LAATSTE ACTIES</strong><span>bevestigde Focus 2.0 audit</span></header>{actions.slice(0,20).map((raw,index)=>{const row=raw as Record<string,unknown>;return <div key={`${row.timestampMs||index}:${row.event||index}`}><time>{Number(row.timestampMs)?new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(new Date(Number(row.timestampMs))):"—"}</time><b>{focusActionLabel(row.event)}</b><span>{Number(row.price)>0?amount(row.price):"—"}</span><span>{Number(row.quantity)>0?amount(row.quantity):"—"}</span><em>UITGEVOERD</em></div>})}</div>}</section>;
}
