...
export function AsterRecentTrades({ snapshot, onRetry }: { snapshot: ExchangeSnapshot; onRetry: () => void }) {
  ...
  const profitPositions = useMemo(() => topProfitPositions(allPositions) as OpenPosition[], [allPositions]);
  const lossPositions = useMemo(() => [...allPositions].filter(position => finite(position.quantity) && Number(position.quantity) > 0).sort((a, b) => (finite(a.unrealizedPnl) ?? 0) - (finite(b.unrealizedPnl) ?? 0)), [allPositions]);
  const livePositions = useMemo(() => [...allPositions].filter(position => finite(position.quantity) && Number(position.quantity) > 0).sort((a, b) => exchangeTimestampMs(b.openedAt) - exchangeTimestampMs(a.openedAt)), [allPositions]);
