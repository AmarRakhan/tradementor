...
function FocusV2CockpitPanel({value}:{value:FocusV2Cockpit}){
  ...
  return <section className={styles.cockpit} aria-label="Focus 2.0 live cockpit">
    ...
    <div className={styles.cockpitMetrics}>
      {v5?<><span>DCA mode <b>{value.dcaMode||"—"}</b></span><span>Hedge state <b>{value.hedgeState||"—"}</b></span><span>Hedge target qty <b>{amount(value.hedgeTargetQty)}</b></span><span>Release herstel <b>{(Number(value.hedgeReleaseRecoveryPct||0)*100).toFixed(2)}%</b></span></>:<><span>Recovery low <b>{amount(value.recoveryLow)}</b></span><span>Recovery high <b>{amount(value.recoveryHigh)}</b></span></>}
