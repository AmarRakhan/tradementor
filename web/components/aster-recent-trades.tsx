...
const FILTERS: { key: FilterKey; label: string }[] = [
  { key: "live", label: "Live" }, { key: "mostDca", label: "Meeste DCA" }, { key: "entered", label: "Ingestapt" }, { key: "closed", label: "Gesloten" },
...
function FocusV2CockpitPanel({value}:{value:FocusV2Cockpit}){
  ...
  return <section className={styles.cockpit} aria-label="Focus 2.0 live cockpit">
    ...
    <div className={styles.cockpitGrid}>
      ...
      <article><span>SHORT / TARGET</span><strong>{money(value.shortNotional)} / {money(value.targetShortNotional)}</strong><small>{amount(value.shortQuantity)} SHORT{v5?"":` · armed ${amount(value.armedRehedgeQty)}`}</small></article>
    ...
    {v5?<div className={styles.cockpitRecovery}><div><span>Laatste DCA</span><b>{Number(value.lastDcaFillPrice)>0?amount(value.lastDcaFillPrice):"—"}</b></div><div><span>Volgende DCA</span><b>{Number(value.nextLongDcaPrice)>0?amount(value.nextLongDcaPrice):"—"}</b></div><div><span>SHORT release</span><b>{Number(value.hedgeReleasePrice)>0?amount(value.hedgeReleasePrice):"—"}</b></div><div><span>Herstel</span><b>{(Number(value.recoverySinceLastDcaPct||0)*100).toFixed(2)}% / {(Number(value.hedgeReleaseRecoveryPct||0)*100).toFixed(2)}%</b></div></div>:simple?<div className={styles.cockpitRecovery}><div><span>Bescherming</span><b className={Number(value.shortQuantity||0)>0?styles.cockpitOk:styles.cockpitWait}>{Number(value.shortQuantity||0)>0?"SHORT ACTIEF":"SHORT VRIJ"}</b></div><div><span>Hersteltrigger</span><b>{amount(value.recoveryTrigger||value.recoveryReboundPrice)}</b></div><div><span>5m safety</span>{value.bollinger5mConfirmed||value.bollinger5mMiddle===0?check(value.bollinger5mConfirmed):<b>OPTIONEEL</b>}</div><div><span>Exchange backup</span><b className={value.rehedgeArmed?styles.cockpitOk:styles.cockpitWait}>{value.rehedgeArmed?"GEWAPEND":"NIET GEWAPEND"}</b></div></div>:fast?<div className={styles.cockpitRecovery}>
    ...
    <div className={styles.cockpitMetrics}>
      {v5?<><span>Cycle status <b>{value.cycleStatus||value.status||"—"}</b></span><span>DCA mode <b>{value.dcaMode||"—"}</b></span><span>Hedge state <b>{value.hedgeState||"—"}</b></span><span>Hedge target qty <b>{amount(value.hedgeTargetQty)}</b></span><span>Release herstel <b>{(Number(value.hedgeReleaseRecoveryPct||0)*100).toFixed(2)}%</b></span>{v6&&<><span>Starthedge <b>{(Number(value.startHedgePercent||0)*100).toFixed(0)}%</b></span><span>Hedge target <b>{(Number(value.hedgeTargetPercent||0)*100).toFixed(0)}%</b></span></>}</>:<><span>Recovery low <b>{amount(value.recoveryLow)}</b></span><span>Recovery high <b>{amount(value.recoveryHigh)}</b></span></>}
      {v5?<><span>LONG break-even <b>{amount(value.longBreakEvenPrice)}</b></span><span>Actuele SHORT <b>{amount(value.shortQuantity)}</b></span><span>Onderste trigger <b>{Number(value.nextLongDcaPrice)>0?`VOLGENDE DCA + SHORT BIJKOPEN · ${amount(value.nextLongDcaPrice)}`:"—"}</b></span><span>Bovenste trigger <b>{Number(value.shortQuantity)>0&&Number(value.hedgeReleasePrice)>0?`SHORT VOLLEDIG LOSLATEN · ${amount(value.hedgeReleasePrice)}`:"Nog geen hedge actief"}</b></span></>:<><span>LONG break-even <b>{amount(value.longBreakEvenPrice)}</b></span><span>{simple?"Winsttrigger":"Volgende release"} <b>{simple?money(value.profitTriggerUsdt):Number(value.nextShortReleasePrice)>0?amount(value.nextShortReleasePrice):"—"}</b></span><span>{simple?"Hersteltrigger":"Release quantity"} <b>{simple?amount(value.recoveryTrigger||value.recoveryReboundPrice):amount(value.nextShortReleaseQty)}</b></span><span>Totaal vrijgegeven <b>{amount(value.releasedShortQty)}</b></span><span>Re-hedge <b>{value.rehedgeArmed?`${amount(value.rehedgePrice)} · ${amount(value.armedRehedgeQty)} SHORT`:"Niet gewapend"}</b></span><span>Volgende DCA <b>{Number(value.nextLongDcaPrice)>0?`${amount(value.nextLongDcaPrice)} · ${Number(value.nextLongDcaDistancePct||0).toFixed(2)}%`:"—"}</b></span></>}
      ...
      {v6?<><span>Take Profit <b>{String(value.tpMode||"usdt").toLowerCase()==="percent"?`${(Number(value.tpValue||0)*100).toFixed(2)}%`:money(value.tpValue)}</b></span><span>Nog tot TP <b>{String(value.tpMode||"usdt").toLowerCase()==="percent"?`${(Number(value.distanceToTp||0)*100).toFixed(2)}%`:money(value.distanceToTp)}</b></span><span>Auto-herstart <b>{value.autoRestart?"AAN":"UIT"}</b></span></>:simple?<><span>Winst sinds harvest <b>{money(value.profitSinceHarvest,true)} / {money(value.profitTriggerUsdt)}</b></span><span>Nog tot afromen <b>{money(value.profitRemainingUsdt)} · volgende ±{money(value.profitHarvestUsdt)}</b></span><span>Laatste / totaal afgeroomd <b>{money(value.lastHarvestProfit,true)} / {money(value.totalHarvestedProfit,true)}</b></span></>:<><span>Profit sinds harvest <b>{money(value.profitSinceHarvest,true)} / trigger {money(value.profitTriggerUsdt)}</b></span><span>Nog tot harvest <b>{money(value.profitRemainingUsdt)} · neem {money(value.profitHarvestUsdt)}</b></span><span>Laatste / totaal harvest <b>{money(value.lastHarvestProfit,true)} / {money(value.totalHarvestedProfit,true)}</b></span></>}
    </div>
...
export function AsterRecentTrades({ snapshot, onRetry }: { snapshot: ExchangeSnapshot; onRetry: () => void }) {
  ...
  const mostDcaPositions = useMemo(() => [...livePositions].sort((a, b) => {
    const aEntries = positionEntryCount(a); const bEntries = positionEntryCount(b);
  ...
  const datasets = useMemo<Record<FilterKey, TradeCenterRow[]>>(() => ({
    live: livePositions.map(position => rowFromPosition(position)), mostDca: mostDcaPositions.map(position => rowFromPosition(position)), entered: entries.map(trade => rowFromActivity(trade, false, positionsWithMultiDcaCounts, scanActions)), closed: exits.map(trade => rowFromActivity(trade, true, allPositions, scanActions)),
  ...
  const counts: Record<FilterKey, number> = { live: livePositions.length, mostDca: mostDcaPositions.length, entered: entries.length, closed: exits.length, tp: tpActions.length, dca: dcaActions.length, profit: profitPositions.length, loss: lossPositions.length, actions: scanActions.length };
  ...
  const accountDisplay = useMemo(() => deriveAsterAccountDisplay({ data: snapshot.data, serverConfirmed: snapshot.serverConfirmed, error: snapshot.error, updatedAt: snapshot.updatedAt }), [snapshot.data, snapshot.serverConfirmed, snapshot.error, snapshot.updatedAt]);
  ...
  const detailPlannedLevels:PlannedActionLevel[]=[
    ...(detailNextDcaPrice&&detailNextDcaPrice>0?[{key:"dca" as const,price:detailNextDcaPrice,label:`DCA ${Math.round(Number(detailNextDcaNumber||1))}`,color:"#ffd166"}]:[]),
    ...(detailTpPrice&&detailTpPrice>0?[{key:"tp" as const,price:detailTpPrice,label:`TP ${detailTpPct!==null?`${detailTpPct.toFixed(2).replace(".",",")}%`:""}`,color:"#58f0ae"}]:[]),
  ];
  ...
  function closeDetail() { detailTapRef.current={at:0,x:0,y:0}; detailTouchRef.current.valid=false; setDetail(null); requestAnimationFrame(() => window.scrollTo({ top: scrollYRef.current, behavior: "auto" })); }
  function detailTargetIsInteractive(target: EventTarget | null) { return target instanceof Element && Boolean(target.closest("button,a,input,select,textarea,[role=button]")); }
  function handleDetailTouchStart(event: TouchEvent<HTMLDivElement>) {
    if (event.touches.length !== 1 || detailTargetIsInteractive(event.target)) { detailTouchRef.current.valid=false; return; }
    const touch=event.touches[0]; detailTouchRef.current={valid:true,x:touch.clientX,y:touch.clientY};
  }
  function handleDetailTouchMove(event: TouchEvent<HTMLDivElement>) {
    if (!detailTouchRef.current.valid || event.touches.length !== 1) { detailTouchRef.current.valid=false; return; }
    const touch=event.touches[0]; if (Math.hypot(touch.clientX-detailTouchRef.current.x,touch.clientY-detailTouchRef.current.y)>10) detailTouchRef.current.valid=false;
  }
  function handleDetailTouchEnd(event: TouchEvent<HTMLDivElement>) {
    if (!detailTouchRef.current.valid || event.changedTouches.length !== 1) { detailTouchRef.current.valid=false; return; }
    detailTouchRef.current.valid=false; const touch=event.changedTouches[0]; const now=Date.now(); const previous=detailTapRef.current;
    const isDouble=now-previous.at<=360 && Math.hypot(touch.clientX-previous.x,touch.clientY-previous.y)<=28;
    detailTapRef.current=isDouble?{at:0,x:0,y:0}:{at:now,x:touch.clientX,y:touch.clientY}; if (isDouble) closeDetail();
  }
  ...
  return <section className={`${styles.shell} ${styles.detailShell} ${detail ? styles.detailOpen : ""}`} aria-label="Aster Tradecentrum"><div className={styles.detailInner}><div className={styles.front} aria-hidden={Boolean(detail)} inert={detail ? true : undefined}><article className={styles.card}><header className={styles.header}><div className={styles.title}><h2>Tradecentrum</h2><p>Eén overzicht · negen live filters</p></div><span className={`${styles.live} ${styles[liveState.toLowerCase()] || ""}`}>{liveState}</span></header><nav className={styles.filters} aria-label="Tradecentrum filters">{FILTERS.map(filter => <button type="button" key={filter.key} className={`${styles.filter} ${active === filter.key ? styles.active : ""}`} aria-pressed={active === filter.key} onClick={() => selectFilter(filter.key)}>{filter.label}<b>{counts[filter.key]}</b></button>)}</nav><TradeCenterTable rows={visibleRows} onOpenDetail={openDetail} onClosed={onRetry} showDcaCount={active === "mostDca"} /><footer className={styles.footer}>{selectedRows.length > 6 && <button type="button" onClick={() => { setExpanded(value => !value); setPages(1); }}>{expanded ? "Compact tonen" : "Toon alles"}</button>}{expanded && hasMore && <button type="button" onClick={() => setPages(value => value + 1)}>Laad nog 100</button>}</footer></article></div><div className={styles.back} aria-hidden={!detail} inert={!detail ? true : undefined} onTouchStart={handleDetailTouchStart} onTouchMove={handleDetailTouchMove} onTouchEnd={handleDetailTouchEnd} onTouchCancel={()=>{detailTouchRef.current.valid=false}}>{detail && <><header className={styles.detailHeader}><div><span>ASTER · TRADEDETAIL</span><h2>{baseAsset(detail.selection.symbol)} <i>/ USDT</i></h2><small>{detail.linkedFromAirbag?`Je bekijkt de Airbag van ${baseAsset(detail.selection.symbol)} ${detailMainSide}`:`HOOFDPOSITIE · ${detailMainSide}`} · dubbel tikken of × om terug te gaan</small></div><button type="button" onClick={closeDetail} aria-label="Terug naar Tradecentrum">×</button></header><section className={styles.tradeNow} aria-label="Actuele Strategy 2 tradegegevens"><div className={styles.tradeNowTop}><div><span>HUIDIGE PNL</span><strong className={(detailPnl ?? 0) >= 0 ? styles.profit : styles.loss}>{money(detailPnl,true)}</strong></div><div><span>MARGIN IN TRADE</span><strong>{money(detailMargin)}</strong></div><div><span>WINST BIJ TP</span><strong className={styles.profit}>{money(detailExpectedPnlAtTp,true)}</strong></div><div><span>PORTFOLIO BIJ TP</span><strong className={styles.profit}>{money(detailPortfolioAtTp)}</strong></div></div><div className={styles.tradeNowBottom}><span>Portfoliowaarde <b>{accountDisplay.equity}</b></span><span>Available to trade <b>{accountDisplay.available}</b></span></div><div className={styles.tradeNowActions}><div className={styles.dcaAction}><span>DCA STATUS</span><strong>Volgende DCA: {Math.round(Number(detailNextDcaNumber||1))}</strong><small>{detailFilledDca} gevuld</small></div><div className={styles.tpAction}><span>TP INSTELLING</span><strong>{detailTpPct===null?"—":`${detailTpPct.toFixed(2).replace(".",",")}%`}</strong><small>Volledige sluiting</small></div></div><div className={styles.tradeNowDistances}><div><span>AFSTAND TOT DCA ({Math.round(Number(detailNextDcaNumber||1))})</span><strong className={styles.dcaTone}>{detailDcaDistanceLabel}</strong></div><div><span>AFSTAND TOT TP ({detailTpPct===null?"—":`${detailTpPct.toFixed(2).replace(".",",")}%`})</span><strong className={styles.profit}>{detailTpDistanceLabel}</strong></div><div><span>DCA GEVULD</span><strong>{detailFilledDca}</strong></div></div></section><SafeTradingChart selection={detail.selection} mode="aster-detail" focusAtMs={detail.focusAtMs} breakEvenPrice={detailBreakEvenPrice} dcaLevels={detailChartDcaLevels} plannedActionLevels={detailPlannedLevels} selectedActionId={detail.selectedActionId} airbagEvents={detailAirbagEvents} cockpit={detailFocusV2Cockpit} accountDisplay={accountDisplay} />{detailFocusV2Cockpit&&<FocusV2CockpitPanel value={detailFocusV2Cockpit}/>}<div className={styles.summary}><div><span>Positie</span><strong className={detail.status === "OPEN" ? styles.profit : styles.neutral}>{detail.status || "—"}</strong></div><div><span>Entry prijs (gem.)</span><strong>{money(detailAverageEntry)}</strong></div><div><span>{detail.status === "CLOSED" ? "Exit prijs" : "Huidige prijs"}</span><strong>{money(detail.status === "CLOSED" ? detail.exitPrice : detailCurrentPrice)}</strong></div><div><span>{detail.status === "CLOSED" ? "Realized P&L" : "Unrealized P&L"}</span><strong className={(detailPnl ?? 0) >= 0 ? styles.profit : styles.loss}>{money(detailPnl, true)}</strong></div><div><span>Margin in trade</span><strong>{money(detailMargin)}</strong></div><div><span>Aantal DCA</span><strong>{detailDcaCount === null || detailDcaCount === undefined ? "—" : Math.round(detailDcaCount)}</strong></div><div><span>Volgende {detailMainSide} DCA prijs</span><strong>{detailNextDcaPrice?`${money(detailNextDcaPrice)} · DCA ${Math.round(Number(detailNextDcaNumber||1))}`:"Geen volgende DCA beschikbaar"}</strong></div><div><span>Afstand tot volgende DCA</span><strong>{detailNextDcaPrice?detailDcaDistanceLabel:"Geen volgende DCA beschikbaar"}</strong></div><div><span>Take Profit ingesteld</span><strong>{detailTpPct===null?"—":`${detailTpPct.toFixed(2).replace(".",",")}%`}</strong></div><div><span>TP prijs</span><strong>{money(detailTpPrice)}</strong></div><div><span>Afstand tot TP</span><strong>{detailTpPrice?detailTpDistanceLabel:"—"}</strong></div><div><span>Verwachte winst bij TP</span><strong className={styles.profit}>{money(detailExpectedPnlAtTp,true)}</strong></div><div><span>Portfoliowaarde bij TP</span><strong>{money(detailPortfolioAtTp)}</strong></div></div>{detailAirbag&&<section className={styles.airbag} aria-label="Portfolio Airbag"><header><div><span>PORTFOLIO AIRBAG</span><strong>{String(detailAirbag.status||"WACHT")}</strong></div><b>{((finite(detailAirbag.hedgeRatio)||0)*100).toFixed(0)}%</b></header><div className={styles.airbagGrid}><div><span>Hoofdpositie</span><strong>{String(detailAirbag.mainSide||detail.selection.side)} · {money(detailAirbag.mainNotional)}</strong></div><div><span>AIRBAG / HEDGE</span><strong>{detailHedgeSide||"—"} · {money(detailAirbag.hedgeNotional)} · BOT BEHEERT</strong></div><div><span>Hedge bijdrage</span><strong className={(finite(detailAirbag.hedgePnl)||0)>=0?styles.profit:styles.loss}>{money(detailAirbag.hedgePnl,true)}</strong></div><div><span>Gecombineerd P&L</span><strong className={(finite(detailAirbag.combinedPnl)||0)>=0?styles.profit:styles.loss}>{money(detailAirbag.combinedPnl,true)}</strong></div></div><p><b>Waarom:</b> {String(detailAirbag.reason||"Geen beschermingsactie nodig")}</p><p><b>Volgende actie:</b> {String(detailAirbag.nextAction||"Hedge stabiel")}</p><p><b>Volgende {detailMainSide} DCA:</b> {detailNextDca?`${money(detailNextDca.price)} · DCA ${Math.round(detailNextDca.number)}`:"Geen geldige server-side DCA beschikbaar"}</p>{detailAirbagEvents.length>0&&<div className={styles.airbagTimeline}>{detailAirbagEvents.slice(-10).reverse().map((event,index)=><div key={`${event.at}:${index}`}><time>{new Intl.DateTimeFormat("nl-NL",{hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date(event.at))}</time><b>{event.kind}</b><span>{(Number(event.ratio)*100).toFixed(0)}%</span></div>)}</div>}</section>}</>}</div></div></section>;
}
