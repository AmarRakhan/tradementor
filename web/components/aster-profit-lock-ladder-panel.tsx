...
export function AsterProfitLockLadderPanel({
  ...
  return <section className={`pll-settings ${enabled ? "is-on" : "is-off"}`} data-reference={REFERENCE}>
    ...
    <style>{`
      #profit-lock-ladder-settings-host{grid-column:1/-1!important;width:100%!important;min-width:0!important;max-width:none!important;justify-self:stretch!important}
      .pll-settings{width:100%;min-width:0;grid-column:1/-1;border:1px solid rgba(214,181,90,.24);border-radius:12px;background:linear-gradient(180deg,rgba(6,20,16,.96),rgba(2,9,7,.96));overflow:hidden;transition:border-color .18s,box-shadow .18s}.pll-settings.is-on{border-color:#16e99a;box-shadow:0 0 0 1px rgba(22,233,154,.16),0 0 24px rgba(22,233,154,.08)}
      ...
      @media(max-width:520px){.pll-expanded{grid-template-columns:1fr}.pll-rules,.pll-explain,.pll-warning,.pll-ladder-head,.pll-table,.pll-direction,.pll-note{grid-column:1}.pll-tr{grid-template-columns:34px minmax(0,1.25fr) minmax(0,.9fr) 74px}.pll-direction{grid-template-columns:1fr}.pll-toggle-row b{font-size:12px}}
    `}</style>
  </section>;
}
