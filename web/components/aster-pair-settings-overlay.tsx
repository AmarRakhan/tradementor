...
const allowedFields = new Set<FieldKey>([
  "entryMarginUsd", "minimumLeverage", "dcaMarginUsd", "dcaDistance", "maxDca", "takeProfit", "takeProfitEnabled", "shortStartMultiplier",
]);
...
function currentPairFromDom() {
  ...
  return compact.endsWith("USDT") ? compact : compact ? `${compact}USDT` : "";
}
...
function parsePositive(value: string, label: string) {
  ...
  if (!Number.isFinite(number) || number <= 0) throw new Error(`${label} moet groter dan 0 zijn.`);
  return number;
}

function overrideKeys(override: Override) {
  return new Set<FieldKey>(Object.keys(override).filter((key) => allowedFields.has(key as FieldKey)) as FieldKey[]);
}
...
export function AsterPairSettingsOverlay() {
  ...
  useEffect(() => {
    ...
    return () => {
      stopped = true;
      observer.disconnect();
      document.querySelectorAll("[data-pair-settings-pencil]").forEach((node) => node.remove());
    };
  }, [launch]);
  ...
  async function save() {
    ...
    try {
      ...
      if (!customEnabled) {
        ...
        if (Object.keys(override).length === 1) delete nextOverrides[symbol]; else nextOverrides[symbol] = override;
      }
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(0,0,0,.78)", backdropFilter: "blur(8px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }} role="dialog" aria-modal="true" aria-label={`${pairLabel} pair-instellingen`}>
      <section style={{ width: "min(100%,680px)", maxHeight: "90dvh", overflowY: "auto", borderRadius: "24px 24px 0 0", border: "1px solid rgba(88,240,174,.22)", background: "linear-gradient(180deg,#07100e 0%,#020706 100%)", color: "#f5f7f6", padding: "20px 18px calc(24px + env(safe-area-inset-bottom))", boxShadow: "0 -24px 80px rgba(0,0,0,.65)" }}>
        <header style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "start", marginBottom: 16 }}>
          <div>
            <small style={{ color: "#58f0ae", letterSpacing: ".14em", fontWeight: 800 }}>PAIR OVERRIDE</small>
            <h2 style={{ margin: "5px 0 2px", fontSize: 25 }}>{pairLabel} · instellingen</h2>
            <p style={{ margin: 0, color: "#9aa7a1", fontSize: 13 }}>Alleen deze munt wijkt af. Niet-aangevinkte velden blijven de basissettings volgen.</p>
          </div>
          <button type="button" aria-label="Sluiten" onClick={() => setOpen(false)} style={{ width: 42, height: 42, flex: "0 0 auto", borderRadius: 13, border: "1px solid #32423b", background: "#152019", color: "white", fontSize: 24 }}>×</button>
        </header>
        ...
        <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "14px", borderRadius: 15, border: "1px solid #24372e", background: "rgba(255,255,255,.025)", marginBottom: 12 }}>
          ...
          <input type="checkbox" checked={customEnabled} onChange={(event) => setCustomEnabled(event.target.checked)} style={{ width: 24, height: 24, accentColor: "#58f0ae" }} />
        </label>
        ...
        <div style={{ display: "grid", gap: 9, opacity: customEnabled ? 1 : .48, pointerEvents: customEnabled ? "auto" : "none" }}>
          {numericFields.map(({ key, label, suffix, nextCycle }) => (
            <div key={key} style={{ display: "grid", gridTemplateColumns: "28px 1fr minmax(105px,145px)", gap: 9, alignItems: "center", padding: "10px 11px", border: enabledFields.has(key) ? "1px solid rgba(88,240,174,.38)" : "1px solid #1b2923", borderRadius: 13, background: enabledFields.has(key) ? "rgba(88,240,174,.055)" : "rgba(255,255,255,.018)" }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700 }}>
                {label}
                {nextCycle && <small style={{ display: "block", color: "#c6a55a", fontWeight: 600, marginTop: 2 }}>Geldt vanaf volgende nieuwe cyclus</small>}
              </span>
              <label style={{ display: "flex", alignItems: "center", gap: 5, border: "1px solid #31463b", borderRadius: 9, padding: "7px 8px", background: "#020604" }}>
                <input
                  value={draft[key]}
                  inputMode="decimal"
                  onFocus={() => { if (!enabledFields.has(key)) setEnabledFields((current) => new Set(current).add(key)); }}
                  onChange={(event) => {
                    setDraft((current) => ({ ...current, [key]: event.target.value }));
                    if (!enabledFields.has(key)) setEnabledFields((current) => new Set(current).add(key));
                  }}
                  style={{ width: "100%", minWidth: 0, border: 0, outline: 0, background: "transparent", color: "white", fontSize: 15, fontWeight: 800, textAlign: "right" }}
                />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "28px 1fr auto", gap: 9, alignItems: "center", padding: "10px 11px", border: enabledFields.has("takeProfitEnabled") ? "1px solid rgba(88,240,174,.38)" : "1px solid #1b2923", borderRadius: 13 }}>
            <input type="checkbox" aria-label="Take Profit actief overschrijven" checked={enabledFields.has("takeProfitEnabled")} onChange={() => toggleField("takeProfitEnabled")} style={{ width: 20, height: 20, accentColor: "#58f0ae" }} />
            <b style={{ fontSize: 13 }}>Take Profit actief</b>
            <input type="checkbox" checked={draft.takeProfitEnabled} onChange={(event) => { setDraft((current) => ({ ...current, takeProfitEnabled: event.target.checked })); setEnabledFields((current) => new Set(current).add("takeProfitEnabled")); }} style={{ width: 24, height: 24, accentColor: "#58f0ae" }} />
          </div>
        </div>
        ...
        {message && <p role="status" style={{ padding: "10px 12px", borderRadius: 10, background: "rgba(88,240,174,.07)", color: /mislukt|ongeldig|moet|niet bevestigd/i.test(message) ? "#ff8d9e" : "#8cf5bd", fontSize: 13 }}>{message}</p>}
