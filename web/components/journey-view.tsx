 ...
 return <section className="journey2026">
  <section className="j26Hero">
   ...
   <div className="j26HeroGrid">
    <div className="j26GoalCopy"><span>Jouw reis naar</span><strong>{usd(goal)}</strong><small>1 level = gemiddelde daggroei</small><div style={{marginTop:8,padding:"10px 12px",border:"1px solid rgba(87,230,164,.20)",borderRadius:12,background:"rgba(5,18,14,.64)",boxShadow:"inset 0 1px rgba(255,255,255,.02)"}}><span style={{display:"block",color:"#8b9692",fontSize:8,letterSpacing:".08em",textTransform:"uppercase"}}>Gemiddelde groei per dag</span><strong style={{display:"block",marginTop:3,color:"var(--jg)",fontSize:24,lineHeight:1,fontWeight:850}}>{daily?.reliable?`${avg.toLocaleString("nl-NL",{minimumFractionDigits:2,maximumFractionDigits:2})}%`:"—"}</strong></div><b>{goalProgress.toLocaleString("nl-NL",{minimumFractionDigits:2,maximumFractionDigits:2})}%</b><i><em style={{width:`${goalProgress}%`}}/></i><small>{usd(equity)} / {usd(goal)}</small></div>
