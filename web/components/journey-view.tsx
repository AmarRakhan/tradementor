...
export function JourneyView({snapshots}:{snapshots:ExchangeSnapshots}){
 ...
 return <section className="journey2026">
  <section className="j26Hero">
   <div className="j26Sky" aria-hidden="true"/><div className="j26Water" aria-hidden="true"/>
   ...
   <div className="j26Destination"><span>EINDDOEL</span><strong>{usd(goal)}</strong></div>
   <div className="j26HeroGrid">
    <div className="j26GoalCopy"><span>Jouw reis naar</span><strong>{usd(goal)}</strong><div className="j26Average"><span>Gemiddelde groei per dag</span><strong>{daily?.reliable?`${avg.toLocaleString("nl-NL",{minimumFractionDigits:2,maximumFractionDigits:2})}%`:"—"}</strong><small>1 level = gemiddelde daggroei</small></div><div className="j26GoalProgress"><b>{goalProgress.toLocaleString("nl-NL",{minimumFractionDigits:2,maximumFractionDigits:2})}%</b><i><em style={{width:`${goalProgress}%`}}/></i><small>{usd(equity)} / {usd(goal)}</small></div></div>
   ...
   <div className="j26Stats"><article><Icon name="goal"/><div><span>DOEL</span><strong>{usd(goal)}</strong></div></article><article><Icon name="earned"/><div><span>TOTAAL VERDIEND</span><strong className={totalEarned>=0?"pos":"neg"}>{baseline?usd(totalEarned):"—"}</strong><small>vandaag</small></div></article><article><Icon name="remaining"/><div><span>RESTEREND TOT DOEL</span><strong>{usd(remaining)}</strong></div></article><article><Icon name="forecast"/><div><span>PROGNOSE</span><strong>{forecast}</strong><small>{forecastDays===null?"—":`Over ${forecastDays} dagen`}</small></div></article></div>
  </section>
  <section className="j26Calendar"><header><div><b>JOURNEY KALENDER</b><span>JOUW DAGELIJKSE VOORTGANG</span></div><small>Swipe om meer te zien</small></header>
   <div className="j26Carousel"><button type="button" className="j26Arrow prev" aria-label="Vorige vier dagen" onClick={()=>shift(-1)}>‹</button><div className="j26Strip" ref={stripRef}>{history.map(d=>{const p=parts(d.date),tone=d.percentage>0?"positive":d.percentage<0?"negative":"neutral";return <button type="button" key={d.date} className={`j26Day ${tone} ${selected===d.date?"selected":""}`} onClick={()=>setSelected(d.date)} aria-pressed={selected===d.date}><b>{p.day}</b><span>{p.month}</span><i>{d.percentage>0?"↑":d.percentage<0?"↓":"–"}</i><strong>{d.levels} {d.levels===1?"level":"levels"}</strong><em>{usd(d.usdChange)}</em><small>{pct(d.percentage)}</small><u>{d.percentage>0?"Groei":d.percentage<0?"Daling":"Gelijk"}</u></button>})}</div><button type="button" className="j26Arrow next" aria-label="Volgende vier dagen" onClick={()=>shift(1)}>›</button></div>
