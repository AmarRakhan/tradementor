...
export function AsterBotConfiguratorV2({ snapshot, serverConfirmed, onConfirmed, onChanged, release }: Props) {
  ...
  return <article id="bot-configurator-v2" data-reference={VISUAL_REFERENCE}>
    ...
    <section className="v2-step" id="v2-step-instap"><StepHead number="3" title="Instaplogica" subtitle="Wanneer mag een nieuwe positie openen?" />
      ...
      <Toggle label="Automatische exposure-refill" description={exposureAvailable ? "Optioneel: alleen de ontbrekende kant krijgt tijdelijk het snelle Bollinger-timeframe." : "Nog niet vrijgegeven."} checked={draft.exposureRefillEnabled} onChange={(v) => update("exposureRefillEnabled", v)} disabled={!exposureAvailable} />
      ...
      <div className="v2-runtime-strip"><span><small>Netto exposure</small><b className={netExposure < 0 ? "short" : netExposure > 0 ? "long" : ""}>{money(netExposure)}</b></span><span><small>Onbalans</small><b>{imbalance.toFixed(1)}%</b></span><span><small>Exposure-refill</small><b className={exposureSide === "LONG" ? "long" : exposureSide === "SHORT" ? "short" : ""}>{exposureRuntimeLabel}</b></span></div>
    </section>
