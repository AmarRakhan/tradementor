...
export function AsterBotConfiguratorV2({ snapshot, serverConfirmed, onConfirmed, onChanged, release }: Props) {
  ...
  return <article id="bot-configurator-v2" data-reference={VISUAL_REFERENCE}>
    ...
    <section className="v2-step" id="v2-step-posities"><StepHead number="2" title="Posities" subtitle="Hoe wil je je portfolio verdelen?" />
      <div className="v2-slot-visual">
        <div className="long"><span>LONG</span><i><u style={{ width: totals.totalSlots ? (totals.longSlots / totals.totalSlots * 100) + "%" : "0%" }} /></i><b>{totals.longSlots}</b></div>
        <div className="short"><span>SHORT</span><i><u style={{ width: totals.totalSlots ? (totals.shortSlots / totals.totalSlots * 100) + "%" : "0%" }} /></i><b>{totals.shortSlots}</b></div>
        <div className="total"><span>TOTAAL</span><strong>{totals.totalSlots}</strong></div>
      </div>
