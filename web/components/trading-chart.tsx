...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  return <section ref={shellRef} className={`trading-terminal ${mode === "aster-detail" ? "aster-detail-chart" : ""}`} aria-label={`Grafiek ${selection.symbol}`}>
    {mode === "aster-detail" ? <><div className="aster-detail-market-strip"><div><strong>{ticker ? ticker.price.toLocaleString("nl-NL",{maximumFractionDigits:8}) : "—"}</strong><span className={ticker && ticker.change >= 0 ? "positive" : "negative"}>{ticker ? `${ticker.change >= 0 ? "+" : ""}${ticker.change.toFixed(2)}%` : "—"}</span></div><small>Focus 2.0 live · {timeframe} · server-triggers</small></div><div className="aster-detail-account-strip" aria-label="Live Aster accountstatus"><div className={`risk-${accountDisplay?.liquidationTone ?? "unknown"}`}><span>LIQUIDITEIT</span><strong>{accountDisplay?.liquidationValue ?? "—"}</strong></div><div><span>PORTFOLIOWAARDE</span><strong>{accountDisplay?.equity ?? "—"}</strong></div><div><span>AVAILABLE TO TRADE</span><strong>{accountDisplay?.available ?? "—"}</strong></div></div><div className="aster-detail-timeframes" role="group" aria-label="Timeframe kiezen">{timeframes.map(tf=><button type="button" key={tf} className={timeframe===tf?"active":""} onClick={()=>setTimeframe(tf)}>{tf}</button>)}{focusV2&&!autoFollow&&<button type="button" className="focus-go-live" onClick={()=>{setAutoFollow(true);requestAnimationFrame(()=>chartRef.current?.timeScale().setVisibleLogicalRange({from:Math.max(0,candleDataRef.current.length-50),to:candleDataRef.current.length+10}))}}>NAAR LIVE</button>}</div>{focusV2&&cockpit&&<div className="focus-cockpit-levels"><span className="current">LIVE {ticker?ticker.price.toLocaleString("nl-NL",{maximumFractionDigits:8}):"—"}</span>{Number(cockpit.longBreakEvenPrice)>0&&<span className="breakeven">LONG GROEN {Number(cockpit.longBreakEvenPrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}{Number(cockpit.nextLongDcaPrice)>0&&<span className="dca">DCA {Number(cockpit.nextLongDcaPrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}{Number(cockpit.recoveryModelVersion||1)>=3&&Number(cockpit.recoveryTrigger||cockpit.nextShortReleasePrice)>0?<span className="release">HERSTEL {Number(cockpit.recoveryTrigger||cockpit.nextShortReleasePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>:Number(cockpit.nextShortReleasePrice)>0&&<span className="release">{Number(cockpit.recoveryModelVersion||1)>=2?`HERSTEL ${Math.min(4,Number(cockpit.recoveryStage||0)+1)}`:"RELEASE"} {Number(cockpit.nextShortReleasePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}{cockpit.rehedgeArmed&&Number(cockpit.rehedgePrice)>0&&<span className="rehedge">RE-HEDGE {Number(cockpit.rehedgePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}</div>}</> : <>
