...
export function TradingChart({ selection, mode = "default", focusAtMs, averageEntry, selectedActionId }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; averageEntry?: number; selectedActionId?: string }) {
  ...
  useEffect(() => {
    ...
    if (mode === "aster-detail" && Number.isFinite(averageEntry) && Number(averageEntry) > 0) {
      ...
      const entrySeries = chart.addSeries(LineSeries,{color:"#9fb3c8",lineWidth:1,lineStyle:2,priceLineVisible:false,lastValueVisible:false,title:""});
  ...
  return <section ref={shellRef} className={`trading-terminal ${mode === "aster-detail" ? "aster-detail-chart" : ""}`} aria-label={`Grafiek ${selection.symbol}`}>
    {mode === "aster-detail" ? <div className="aster-detail-market-strip"><div><strong>{ticker ? ticker.price.toLocaleString("nl-NL",{maximumFractionDigits:8}) : "—"}</strong><span className={ticker && ticker.change >= 0 ? "positive" : "negative"}>{ticker ? `${ticker.change >= 0 ? "+" : ""}${ticker.change.toFixed(2)}%` : "—"}</span></div><small>Laatste 50 candles · 15m · Bollinger Bands</small></div> : <>
