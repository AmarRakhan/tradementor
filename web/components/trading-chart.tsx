...
export type PlannedActionLevel = { key: "dca" | "dca-long" | "dca-short" | "tp"; price: number; label: string; color?: string };
...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, plannedActionLevels = EMPTY_PLANNED_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; plannedActionLevels?: PlannedActionLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  useEffect(() => {
    ...
    if (mode === "aster-detail" && !focusV2) {
      ...
      plannedActionLevels.filter(level=>(level.key==="dca"||level.key==="dca-long"||level.key==="dca-short")&&Number.isFinite(level.price)&&level.price>0).forEach(level=>priceSeries.createPriceLine({price:Number(level.price),color:level.color||"#ffd166",lineWidth:3,lineStyle:0,axisLabelVisible:false,title:""}));
  ...
  const plannedOverlayLevels=useMemo(()=>mode==="aster-detail"&&!focusV2?plannedActionLevels.filter(level=>Number.isFinite(level.price)&&level.price>0).map(level=>({...level,label:level.label||((level.key==="dca"||level.key==="dca-long"||level.key==="dca-short")?`VOLGENDE ${selection.side.toUpperCase()} DCA`:"TP"),color:level.color||((level.key==="dca"||level.key==="dca-long"||level.key==="dca-short")?"#ffd166":"#b978ff")})):[] as Array<{key:string;price:number;label:string;color:string}>,[mode,focusV2,plannedActionLevelsSignature,selection.side]);
