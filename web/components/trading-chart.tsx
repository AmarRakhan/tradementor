...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  useEffect(() => {
    ...
    if(focusV2&&cockpit){
      ...
      const liveForDistance=Number(ticker?.price||cockpit.currentPrice||0);
      const dcaPrice=Number(cockpit.nextLongDcaPrice||0);
      const liveDcaDistancePct=liveForDistance>0&&dcaPrice>0?((dcaPrice/liveForDistance)-1)*100:Number(cockpit.nextLongDcaDistancePct);
      addLevel(cockpit.nextLongDcaPrice,"#ffd166",`${hedgeActive?"LONG DCA + SHORT BIJKOPEN":"VOLGENDE LONG DCA"}${Number.isFinite(liveDcaDistancePct)?` · ${liveDcaDistancePct.toFixed(2)}%`:""}`,0,2);
      if(stateVersion>=5){
        if(hedgeActive&&Number(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice)>0)addLevel(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice,"#ff9f43",`SHORT LOSLATEN · +${(Number(cockpit.hedgeReleaseRecoveryPct||0)*100).toFixed(2)}%`,0,2);
      } else if(Number(cockpit.recoveryModelVersion||1)>=3){
      ...
      if(stateVersion<5&&Number(cockpit.recoveryModelVersion||1)<3&&cockpit.rehedgeArmed)addLevel(cockpit.rehedgePrice,"#ff5f7d","RE-HEDGE · GEWAPEND",2,2);
