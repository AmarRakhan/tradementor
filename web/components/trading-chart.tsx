...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  return <section ref={shellRef} className={`trading-terminal ${mode === "aster-detail" ? "aster-detail-chart" : ""}`} aria-label={`Grafiek ${selection.symbol}`}>
    ...
    <div className="chart-stage">{loading&&<div className="chart-state"><i/>Candles laden…</div>}{error&&<div className="chart-state error"><strong>Marktdata tijdelijk niet beschikbaar</strong><span>{error}</span><button onClick={load}>Opnieuw proberen</button></div>}<div ref={containerRef} className="chart-canvas" />{focusV2&&focusLevels.length>0&&<div className="focus-level-overlay" aria-hidden="true">{focusLevels.map(level=>Number.isFinite(focusLevelY[level.key])?<div key={level.key} className={`focus-level-segment ${level.key}`} data-focus-chart-label={level.key} style={{top:`${focusLevelY[level.key]}px`,color:level.color,left:"12px",right:"88px",maxWidth:"none",overflow:"hidden"}}><i style={{minWidth:0,flex:"1 1 auto"}}/><span style={{marginLeft:"8px",flex:"0 0 auto",whiteSpace:"nowrap",maxWidth:"100%"}}>{level.label}</span></div>:null)}</div>}{crosshair&&<div className="ohlc-readout"><span>{new Date(crosshair.time*1000).toLocaleString("nl-NL")}</span><b>O {crosshair.open}</b><b>H {crosshair.high}</b><b>L {crosshair.low}</b><b>C {crosshair.close}</b><b>V {crosshair.volume}</b></div>}</div>
