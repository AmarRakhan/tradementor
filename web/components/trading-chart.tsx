...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  const focusLevels=useMemo(()=>{if(!focusV2||!cockpit)return[] as Array<{key:string;price:number;label:string;color:string}>;const dcaLabel=focusHedgeActive?"DCA / SHORT SYNC":"DCA / FULL SHORT RE-HEDGE";const rows=[{key:"live",price:focusLivePrice,label:"LIVE",color:"#25df91"},{key:"be",price:Number(cockpit.longBreakEvenPrice||0),label:`BREAK-EVEN ${distanceLabel(Number(cockpit.longBreakEvenPrice||0))}`,color:"#4aa3ff"},{key:"dca",price:Number(cockpit.nextLongDcaPrice||0),label:`${dcaLabel} ${distanceLabel(Number(cockpit.nextLongDcaPrice||0))}`,color:"#ffd166"},{key:"tp",price:focusTakeProfitPrice,label:`${focusTpLabel} ${distanceLabel(focusTakeProfitPrice)}`,color:"#b978ff"}];if(focusHedgeActive&&focusReleasePrice>0){const priceReady=Boolean(cockpit.shortReleasePriceReady),netReady=Boolean(cockpit.shortReleaseNetGreenReady);const gate=priceReady?(netReady?"READY · NET GREEN":"PRICE READY · WAIT NET GREEN"):"+0.15%";rows.push({key:"release",price:focusReleasePrice,label:`SHORT RELEASE ${gate} ${distanceLabel(focusReleasePrice)}`,color:"#ff9f43"})}return rows.filter(row=>Number.isFinite(row.price)&&row.price>0)},[focusV2,cockpit,focusLivePrice,focusHedgeActive,focusReleasePrice,focusTakeProfitPrice,focusTpLabel]);
