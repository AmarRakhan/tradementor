...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  return <section ref={shellRef} className={`trading-terminal ${mode === "aster-detail" ? "aster-detail-chart" : ""}`} aria-label={`Grafiek ${selection.symbol}`}>
    ...
    <div className="chart-stage">{loading&&<div className="chart-state"><i/>Candles laden…</div>}{error&&<div className="chart-state error"><strong>Marktdata tijdelijk niet beschikbaar</strong><span>{error}</span><button onClick={load}>Opnieuw proberen</button></div>}<div ref={containerRef} className="chart-canvas" />{focusV2&&focusLevels.length>0&&<div className="focus-level-overlay" aria-hidden="true" data-focus-label-overlay="true">{focusLevels.map(level=>{const layout=focusLevelY[level.key];if(!layout||!Number.isFinite(layout.realY)||!Number.isFinite(layout.labelY))return null;const delta=layout.labelY-layout.realY;return <div key={level.key} className={`focus-level-segment ${level.key}`} data-focus-chart-label={level.key} style={{top:`${layout.realY}px`,color:level.color}}><i className="focus-price-line"/><b className="focus-label-leader" aria-hidden="true" style={{display:Math.abs(delta)>2?"block":"none",top:`${Math.min(0,delta)}px`,height:`${Math.abs(delta)}px`}}/><span className="focus-line-label" style={{top:`${delta}px`}}>{level.label}</span></div>})}</div>}{crosshair&&<div className="ohlc-readout"><span>{new Date(crosshair.time*1000).toLocaleString("nl-NL")}</span><b>O {crosshair.open}</b><b>H {crosshair.high}</b><b>L {crosshair.low}</b><b>C {crosshair.close}</b><b>V {crosshair.volume}</b></div>}</div>
