...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  const focusTakeProfitPrice=useMemo(()=>{
  ...
  useEffect(() => {
    ...
    if(focusV2){
      ...
      addSegment("be","#4aa3ff",2);addSegment("dca","#ffd166",2);addSegment("tp","#b978ff",2);addSegment("rebuild","#ff5f7d",2);addSegment("release","#ff9f43",2);
  ...
  const focusReleasePrice=Number(cockpit?.shortNetGreenReleasePrice||cockpit?.hedgeReleasePrice||cockpit?.recoveryTrigger||cockpit?.nextShortReleasePrice||0);
  ...
  const focusTpLabel=useMemo(()=>{const mode=String(cockpit?.tpMode||"").toLowerCase(),value=Number(cockpit?.tpValue||0);if(!(value>0))return"TP";return mode==="percent"?`TP +${(value*100).toFixed(2)}%`:`TP $${value.toFixed(2)}`},[cockpit]);
  ...
  const focusLevels=useMemo(()=>{if(!focusV2||!cockpit)return[] as Array<{key:string;price:number;label:string;color:string}>;const rows=[{key:"live",price:focusLivePrice,label:"LIVE",color:"#25df91"},{key:"be",price:Number(cockpit.longBreakEvenPrice||0),label:`BREAK-EVEN ${distanceLabel(Number(cockpit.longBreakEvenPrice||0))}`,color:"#4aa3ff"},{key:"dca",price:Number(cockpit.nextLongDcaPrice||0),label:`${focusHedgeActive?"DCA / SHORT FILL":"DCA"} ${distanceLabel(Number(cockpit.nextLongDcaPrice||0))}`,color:"#ffd166"},{key:"tp",price:focusTakeProfitPrice,label:`${focusTpLabel} ${distanceLabel(focusTakeProfitPrice)}`,color:"#b978ff"}];const rebuild=Number(cockpit.shortRebuildPrice||0);if(Boolean(cockpit.shortRebuildArmed)&&!focusHedgeActive&&rebuild>0)rows.push({key:"rebuild",price:rebuild,label:`SHORT REBUILD ${distanceLabel(rebuild)}`,color:"#ff5f7d"});if(focusHedgeActive&&focusReleasePrice>0)rows.push({key:"release",price:focusReleasePrice,label:`SHORT RELEASE NET+ ${distanceLabel(focusReleasePrice)}`,color:"#ff9f43"});return rows.filter(row=>Number.isFinite(row.price)&&row.price>0)},[focusV2,cockpit,focusLivePrice,focusHedgeActive,focusReleasePrice,focusTakeProfitPrice,focusTpLabel]);
