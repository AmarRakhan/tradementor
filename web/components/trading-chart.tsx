...
const EMPTY_PLANNED_LEVELS: PlannedActionLevel[] = [];
...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, plannedActionLevels = EMPTY_PLANNED_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; plannedActionLevels?: PlannedActionLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  const focusLocalTrailRef = useRef<{key:string;anchor:number}>({key:"",anchor:0});
  ...
  const [focusLevelY, setFocusLevelY] = useState<Record<string, {realY:number;labelY:number}>>({});
  ...
  const plannedActionLevelsSignature=useMemo(()=>plannedActionLevels.map(level=>`${level.key}:${Number(level.price)}:${level.label}`).join("|"),[plannedActionLevels]);
  ...
  useEffect(() => {
    ...
    if (mode === "aster-detail" && !focusV2) {
      ...
      plannedActionLevels.filter(level=>level.key.startsWith("dca")&&Number.isFinite(level.price)&&level.price>0).forEach(level=>priceSeries.createPriceLine({price:Number(level.price),color:level.color||"#ffd166",lineWidth:3,lineStyle:0,axisLabelVisible:false,title:""}));
      plannedActionLevels.filter(level=>level.key.startsWith("be-")&&Number.isFinite(level.price)&&level.price>0).forEach(level=>priceSeries.createPriceLine({price:Number(level.price),color:level.color||"#21d6a2",lineWidth:2,lineStyle:2,axisLabelVisible:false,title:""}));
      plannedActionLevels.filter(level=>level.key.startsWith("tp")&&Number.isFinite(level.price)&&level.price>0).forEach(level=>priceSeries.createPriceLine({price:Number(level.price),color:level.color||"#58f0ae",lineWidth:2,lineStyle:0,axisLabelVisible:false,title:""}));
    }
    if (mode === "aster-detail") {
  ...
  },[datasetVersion,chartType,activeIndicators,selection,tradeEventsSignature,skin,mode,focusAtMs,breakEvenPrice,dcaLevelsSignature,plannedActionLevelsSignature,selectedActionId,airbagEventsSignature,focusV2]);
  ...
  const focusV8=Number(cockpit?.stateMachineVersion||0)>=8;
  const focusProtectedBottom=Number(cockpit?.protectedFloorPrice||0);
  const focusServerNextBuy=Number(cockpit?.nextLongDcaPrice||0);
  const focusServerAnchor=Number(cockpit?.dcaAnchorPrice||0);
  const focusDcaRatio=focusServerAnchor>0&&focusServerNextBuy>0?Math.max(0,Math.min(.25,1-(focusServerNextBuy/focusServerAnchor))):0;
  const focusTrailKey=`${String(cockpit?.cycleId||"")}:${Number(cockpit?.dcaCount||0)}`;
  const focusTrailingActive=!focusV8&&String(cockpit?.dcaMode||"").toUpperCase()==="TRAILING"&&!Boolean(cockpit?.dcaTriggerPending)&&focusDcaRatio>0;
  const latestMarketHigh=Number(candles.at(-1)?.high||focusLivePrice||0);
  if(focusLocalTrailRef.current.key!==focusTrailKey)focusLocalTrailRef.current={key:focusTrailKey,anchor:Math.max(0,focusServerAnchor)};
  if(focusTrailingActive)focusLocalTrailRef.current.anchor=Math.max(focusLocalTrailRef.current.anchor,focusServerAnchor,latestMarketHigh,focusLivePrice);
  else focusLocalTrailRef.current.anchor=Math.max(0,focusServerAnchor);
  const focusDisplayAnchor=focusLocalTrailRef.current.anchor;
  const focusRawNextBuy=focusTrailingActive&&focusDisplayAnchor>0?focusDisplayAnchor*(1-focusDcaRatio):(Boolean(cockpit?.dcaTriggerPending)&&Number(cockpit?.dcaTriggerPrice||0)>0?Number(cockpit?.dcaTriggerPrice):focusServerNextBuy);
  ...
  const focusReleasePct=useMemo(()=>{const configured=Number(cockpit?.hedgeReleaseRecoveryPct||0);if(configured>0)return configured*100;const lastFill=Number(cockpit?.lastDcaFillPrice||0);return lastFill>0&&focusReleasePrice>0?Math.abs((focusReleasePrice/lastFill)-1)*100:0},[cockpit,focusReleasePrice]);
  const focusLevels=useMemo(()=>{if(!focusV2||!cockpit)return[] as Array<{key:string;price:number;label:string;color:string}>;const rows=[{key:"live",price:focusLivePrice,label:"LIVE",color:"#25df91"}];if(focusV8&&focusProtectedBottom>0)rows.push({key:"bottom",price:focusProtectedBottom,label:"BODEM / RE-HEDGE",color:"#ffd166"});if(!focusV8&&focusNextBuyInvariantOk)rows.push({key:"dca",price:focusRawNextBuy,label:`TERUGVAL -${(focusDcaRatio*100).toFixed(2)}%`,color:"#ffd166"});const lastFill=Number(cockpit?.lastDcaFillPrice||0);if(lastFill>0)rows.push({key:"lastfill",price:lastFill,label:"LAATSTE KOOP",color:"#4aa3ff"});if(lastFill>0&&focusHedgeActive&&focusReleasePrice>0)rows.push({key:"release",price:focusReleasePrice,label:`RELEASE +${focusReleasePct.toFixed(2)}%`,color:"#ff9f43"});const rehedge=Number(cockpit?.rehedgePrice||0);if(Boolean(cockpit?.rehedgeArmed)&&rehedge>0)rows.push({key:"rehedge",price:rehedge,label:"RE-HEDGE",color:"#f06292"});return rows.filter(row=>Number.isFinite(row.price)&&row.price>0)},[focusV2,cockpit,focusLivePrice,focusNextBuyInvariantOk,focusRawNextBuy,focusNextBuyDistancePct,focusHedgeActive,focusReleasePrice,focusReleasePct]);
  const plannedOverlayLevels=useMemo(()=>mode==="aster-detail"&&!focusV2?plannedActionLevels.filter(level=>Number.isFinite(level.price)&&level.price>0).map(level=>({...level,label:level.label||(level.key.startsWith("dca")?`VOLGENDE ${selection.side.toUpperCase()} DCA`:level.key.startsWith("be-")?"BREAK-EVEN":"TP"),color:level.color||(level.key.startsWith("dca")?"#ffd166":level.key.startsWith("be-")?"#21d6a2":"#58f0ae")})):[] as Array<{key:string;price:number;label:string;color:string}>,[mode,focusV2,plannedActionLevelsSignature,selection.side]);
