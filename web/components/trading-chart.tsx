...
export function TradingChart({ selection, mode = "default", focusAtMs, breakEvenPrice, dcaLevels = EMPTY_DCA_LEVELS, selectedActionId, airbagEvents = [], cockpit, accountDisplay }: { selection: TradeSelection; mode?: TradingChartMode; focusAtMs?: number; breakEvenPrice?: number; dcaLevels?: DcaChartLevel[]; selectedActionId?: string; airbagEvents?: AirbagChartEvent[]; cockpit?: FocusV2Cockpit|null; accountDisplay?: AsterAccountDisplay | null }) {
  ...
  useEffect(() => {
    ...
    for(const placement of ["above","below"] as const){const groups=markerGroups.filter(group=>group.placement===placement);if(!groups.length)continue;const markerSeries=chart.addSeries(LineSeries,{color:"rgba(0,0,0,0)",lineVisible:false,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});markerSeries.setData(groups.map(group=>({time:group.time as UTCTimestamp,value:group.displayPrice})));createSeriesMarkers(markerSeries,groups.map(group=>{const highlighted=(Boolean(selectedActionId)&&group.events.some(event=>event.id===selectedActionId))||(mode==="aster-detail"&&Boolean(focusAtMs)&&group.events.some(event=>Math.abs(event.timestampMs-Number(focusAtMs))<=900_000));const onlyHedge=group.events.every(event=>event.kind==="hedge");const one=group.events[0];const label=mode==="aster-detail"?(onlyHedge?(group.events.some(event=>event.action==="close")?"REL":"HEDGE"):group.events.length>1?`${group.events.length} ACTIES`:one?.kind==="dca"?`DCA${one.dcaNumber?` ${one.dcaNumber}`:""}`:one?.kind==="harvest"?"HARVEST":one?.kind==="close"?"CLOSE":one?.side==="LONG"?"BUY":"SHORT"):(group.events.length>1?String(group.events.length):"");return{time:group.time as UTCTimestamp,position:"inBar",color:highlighted?"#ffd166":onlyHedge?"#55e3ff":group.color,shape:group.shape,text:label,size:onlyHedge?0.9:highlighted?1.8:group.events.some(event=>event.kind!=="dca")?1.35:1.15}}));}
    if(focusV2&&cockpit){
      const lastTime=Number(chartCandles.at(-1)?.time||0);
      const step=timeframeSeconds[timeframe]||Math.max(60,Number(chartCandles.at(-1)?.time||0)-Number(chartCandles.at(-2)?.time||0)||60);
      const segmentEnd=(lastTime+step*10) as UTCTimestamp;
      const addSegment=(price:unknown,color:string,lineWidth:1|2|3|4=2)=>{
        const n=Number(price);if(!Number.isFinite(n)||n<=0||lastTime<=0)return null;
        const series=chart.addSeries(LineSeries,{color,lineWidth,lineStyle:0,priceLineVisible:false,lastValueVisible:true,crosshairMarkerVisible:false});
        series.setData([{time:lastTime as UTCTimestamp,value:n},{time:segmentEnd,value:n}]);
        return series;
      };
      currentPriceLineRef.current=addSegment(cockpit.currentPrice,"#25df91",2);
      addSegment(cockpit.longBreakEvenPrice,"#4aa3ff",2);
      const stateVersion=Number(cockpit.stateMachineVersion||0);
      const hedgeActive=stateVersion>=5&&Number(cockpit.shortQuantity||0)>0&&String(cockpit.hedgeState||"").toUpperCase()!=="OFF";
      addSegment(cockpit.nextLongDcaPrice,"#ffd166",2);
      if(stateVersion>=5){
        if(hedgeActive&&Number(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice)>0)addSegment(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice,"#ff9f43",2);
      } else {
        if(Number(cockpit.recoveryModelVersion||1)>=3&&cockpit.rehedgeArmed)addSegment(cockpit.rehedgePrice,"#ff5f7d",2);
        else addSegment(cockpit.nextShortReleasePrice,"#ff9f43",2);
      }
  ...
  useEffect(()=>{
    ...
    try {
      ...
      if(focusV2&&currentPriceLineRef.current){const step=timeframeSeconds[timeframe]||60;currentPriceLineRef.current.setData([{time:next.time as UTCTimestamp,value:next.close},{time:(next.time+step*10) as UTCTimestamp,value:next.close}]);}
  ...
  },[candles,chartType,focusV2,autoFollow,timeframe]);
  ...
  return <section ref={shellRef} className={`trading-terminal ${mode === "aster-detail" ? "aster-detail-chart" : ""}`} aria-label={`Grafiek ${selection.symbol}`}>
    {mode === "aster-detail" ? <><div className="aster-detail-market-strip"><div><strong>{ticker ? ticker.price.toLocaleString("nl-NL",{maximumFractionDigits:8}) : "—"}</strong><span className={ticker && ticker.change >= 0 ? "positive" : "negative"}>{ticker ? `${ticker.change >= 0 ? "+" : ""}${ticker.change.toFixed(2)}%` : "—"}</span></div><small>Focus 2.0 live · {timeframe} · server-triggers</small></div><div className="aster-detail-account-strip" aria-label="Live Aster accountstatus"><div className={`risk-${accountDisplay?.liquidationTone ?? "unknown"}`}><span>LIQUIDATIERISICO</span><strong>{accountDisplay?.liquidationValue ?? "—"}</strong></div><div><span>PORTFOLIOWAARDE</span><strong>{accountDisplay?.equity ?? "—"}</strong></div><div><span>AVAILABLE TO TRADE</span><strong>{accountDisplay?.available ?? "—"}</strong></div></div><div className="aster-detail-timeframes" role="group" aria-label="Timeframe kiezen">{timeframes.map(tf=><button type="button" key={tf} className={timeframe===tf?"active":""} onClick={()=>setTimeframe(tf)}>{tf}</button>)}{focusV2&&!autoFollow&&<button type="button" className="focus-go-live" onClick={()=>{setAutoFollow(true);requestAnimationFrame(()=>chartRef.current?.timeScale().setVisibleLogicalRange({from:Math.max(0,candleDataRef.current.length-50),to:candleDataRef.current.length+18}))}}>NAAR LIVE</button>}</div>{focusV2&&cockpit&&<div className="focus-cockpit-levels"><span className="current">LIVE {ticker?ticker.price.toLocaleString("nl-NL",{maximumFractionDigits:8}):"—"}</span>{Number(cockpit.longBreakEvenPrice)>0&&<span className="breakeven">BE {Number(cockpit.longBreakEvenPrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}{Number(cockpit.nextLongDcaPrice)>0&&<span className="dca">{Number(cockpit.stateMachineVersion||0)>=5&&Number(cockpit.shortQuantity||0)>0?"DCA + SHORT BIJ":"DCA"} {Number(cockpit.nextLongDcaPrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>}{Number(cockpit.stateMachineVersion||0)>=5&&Number(cockpit.shortQuantity||0)>0&&String(cockpit.hedgeState||"").toUpperCase()!=="OFF"&&Number(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice)>0?<span className="release">SHORT LOS {Number(cockpit.hedgeReleasePrice||cockpit.nextShortReleasePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>:Number(cockpit.stateMachineVersion||0)<5&&(Number(cockpit.recoveryModelVersion||1)>=3&&Number(cockpit.recoveryTrigger||cockpit.nextShortReleasePrice)>0?<span className="release">HERSTEL {Number(cockpit.recoveryTrigger||cockpit.nextShortReleasePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>:Number(cockpit.nextShortReleasePrice)>0&&<span className="release">{Number(cockpit.recoveryModelVersion||1)>=2?`HERSTEL ${Math.min(4,Number(cockpit.recoveryStage||0)+1)}`:"RELEASE"} {Number(cockpit.nextShortReleasePrice).toLocaleString("nl-NL",{maximumFractionDigits:8})}</span>)}</div>}</> : <>
