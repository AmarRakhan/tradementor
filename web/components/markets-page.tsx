...
type SortKey = "price" | "volume" | "change" | "leverage" | "bb";
type SortDirection = "asc" | "desc";
type MarketRow = {
  symbol: string; baseAsset: string; lastPrice: number; change24hPct: number; quoteVolume24h: number;
  maxLeverage: number | null; bbStatus: BbStatus | null; bbUpper: number | null; bbMiddle: number | null; bbLower: number | null;
};
...
const BB_FILTERS: Array<{ id: BbFilter; label: string }> = [
  { id: "all", label: "Alle" }, { id: "above", label: "Boven upper" }, { id: "between", label: "Tussen banden" }, { id: "below", label: "Onder lower" },
];
const BB_RANK: Record<BbStatus, number> = { above: 3, between: 2, below: 1 };
const ENRICH_BATCH_SIZE = 8;

function compactUsd(value: number) { return new Intl.NumberFormat("nl-NL", { style: "currency", currency: "USD", notation: "compact", maximumFractionDigits: 1 }).format(value); }
function price(value: number) { const digits = value >= 1000 ? 2 : value >= 1 ? 4 : value >= .01 ? 5 : 7; return new Intl.NumberFormat("en-US", { maximumFractionDigits: digits }).format(value); }
function signedPercent(value: number) { return `${value > 0 ? "+" : ""}${value.toFixed(2)}%`; }
function statusLabel(status: BbStatus) { return status === "above" ? "Above Upper" : status === "below" ? "Below Lower" : "Between Bands"; }
...
export function MarketsPage() {
    ...
    try {
      ...
      for (let index = 0; index < symbols.length; index += ENRICH_BATCH_SIZE) {
        ...
        try {
          ...
          if (generationRef.current !== generation) return;
          if (!payload || !Array.isArray(payload.markets)) throw new Error("Ongeldige Markets-enrichment");
          ...
          setData((current) => current && current.interval === base.interval ? { ...current, updatedAt: Math.max(current.updatedAt, payload.updatedAt || 0), markets: current.markets.map((row) => { const update = updates.get(row.symbol); return update ? { ...row, ...update, bbStatus: classifyBb(row.lastPrice, update.bbUpper, update.bbLower, update.bbStatus) } : row; }) } : current);
  ...
  const load = useCallback(async (background = false, runEnrichment = true) => {
    ...
    try {
      ...
      setData((current) => {
        ...
        return { ...payload, markets: payload.markets.map((fresh) => { const old = oldBySymbol.get(fresh.symbol); return old ? { ...fresh, maxLeverage: old.maxLeverage, bbUpper: old.bbUpper, bbMiddle: old.bbMiddle, bbLower: old.bbLower, bbStatus: classifyBb(fresh.lastPrice, old.bbUpper, old.bbLower, old.bbStatus) } : fresh; }) };
      });
      setLoading(false); setRefreshing(false); if (runEnrichment) void enrich(payload, generation);
  ...
  const rows = useMemo(() => {
    ...
    const filtered = (data?.markets || []).filter((row) => !normalized || row.symbol.includes(normalized) || row.baseAsset.includes(normalized)).filter((row) => bbFilter === "all" || row.bbStatus === bbFilter);
    const direction = sortDirection === "asc" ? 1 : -1;
    return [...filtered].sort((a, b) => {
      if ((sortKey === "leverage" && (a.maxLeverage === null || b.maxLeverage === null)) || (sortKey === "bb" && (a.bbStatus === null || b.bbStatus === null))) {
        const aReady = sortKey === "leverage" ? a.maxLeverage !== null : a.bbStatus !== null; const bReady = sortKey === "leverage" ? b.maxLeverage !== null : b.bbStatus !== null;
        if (aReady !== bReady) return aReady ? -1 : 1;
      }
      let delta = 0;
      if (sortKey === "price") delta = a.lastPrice - b.lastPrice;
      else if (sortKey === "volume") delta = a.quoteVolume24h - b.quoteVolume24h;
  ...
  const openMarket = (row: MarketRow) => {
    const url = new URL(window.location.href); url.searchParams.delete("tmView"); url.searchParams.set("tmSymbol", row.symbol); url.searchParams.set("tmTimeframe", timeframe); url.hash = "/aster";
    window.history.pushState({ destination: "aster", market: row.symbol, timeframe }, "", `${url.pathname}${url.search}${url.hash}`);
    window.dispatchEvent(new CustomEvent("tradementor:open-aster-market", { detail: { symbol: row.symbol, timeframe } }));
    window.dispatchEvent(new PopStateEvent("popstate", { state: window.history.state }));
  };

  return <section className={styles.page} aria-labelledby="markets-title">
    <header className={styles.heading}><div><span className={styles.eyebrow}>ASTER · USDT PERPETUALS</span><h1 id="markets-title">Markets</h1><p>Actieve Aster-markten, leverage en Bollinger-status.</p></div><button type="button" className={styles.refresh} onClick={() => void load(true, false)} disabled={loading || refreshing}>{refreshing ? "Verversen…" : "↻ Vernieuwen"}</button></header>
    ...
    <div className={styles.sortBar} role="group" aria-label="Markets sortering"><button type="button" onClick={() => chooseSort("price")} className={sortKey === "price" ? styles.activeSort : ""}>Prijs {arrow("price")}</button><button type="button" onClick={() => chooseSort("volume")} className={sortKey === "volume" ? styles.activeSort : ""}>Volume {arrow("volume")}</button><button type="button" onClick={() => chooseSort("change")} className={sortKey === "change" ? styles.activeSort : ""}>24h {arrow("change")}</button><button type="button" onClick={() => chooseSort("leverage")} className={sortKey === "leverage" ? styles.activeSort : ""}>Leverage {arrow("leverage")}</button><button type="button" onClick={() => chooseSort("bb")} className={sortKey === "bb" ? styles.activeSort : ""}>BB {arrow("bb")}</button></div>
    {loading && !data ? <div className={styles.state}><div className={styles.spinner} /><strong>Aster Markets laden</strong></div> : error && !data ? <div className={`${styles.state} ${styles.error}`}><strong>Markets tijdelijk niet beschikbaar</strong><span>{error}</span><button type="button" onClick={() => void load(false, true)}>Opnieuw proberen</button></div> : rows.length === 0 && enriching && bbFilter !== "all" ? <div className={styles.state}><div className={styles.spinner} /><strong>Bollinger-data aanvullen</strong></div> : rows.length === 0 ? <div className={styles.state}><strong>Geen markten gevonden</strong><span>Pas je zoekterm of filter aan.</span></div> : <div className={styles.list} aria-live="polite">{rows.map((row) => <article key={row.symbol} className={styles.row} role="button" tabIndex={0} aria-label={`${row.symbol} openen in Aster Tradecentrum`} onClick={() => openMarket(row)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openMarket(row); } }}><div className={styles.identity}><span className={styles.coin}>{row.baseAsset.slice(0, 2)}</span><div><div className={styles.symbolLine}><strong>{row.symbol}</strong><em>{row.maxLeverage !== null ? `${row.maxLeverage}x` : "—"}</em></div><small>Vol {compactUsd(row.quoteVolume24h)}</small></div></div><div className={styles.marketPrice}><strong>${price(row.lastPrice)}</strong><span className={row.change24hPct > 0 ? styles.positive : row.change24hPct < 0 ? styles.negative : ""}>{signedPercent(row.change24hPct)}</span></div>{row.bbStatus && row.bbUpper !== null && row.bbMiddle !== null && row.bbLower !== null ? <div className={`${styles.bbBadge} ${styles[row.bbStatus]}`} title={`Upper ${price(row.bbUpper)} · Mid ${price(row.bbMiddle)} · Lower ${price(row.bbLower)}`}><i />{statusLabel(row.bbStatus)}</div> : <div className={styles.bbBadge}><i />BB laden</div>}</article>)}</div>}
