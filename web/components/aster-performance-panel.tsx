...
export function AsterPerformancePanel({ snapshot, onChanged }: { snapshot: Record<string, unknown> | null; onChanged: () => void }) {
  ...
  return <section className="aster-performance">
    ...
    <div className="strategy-performance-grid"><article><b>Strategy 2 · Dual Profit Harvest</b><span>{s2.enabled ? "Engine actief" : "Nog niet gestart"} · Live {s2.liveReady ? "ready" : "locked"}</span><small>Dit is de enige actieve Aster-strategie. LONG en SHORT worden als zelfstandige harvestcycles gevolgd.</small></article></div>
    <AsterUniverseStatus value={s2.universe} label="Strategy 2" />
