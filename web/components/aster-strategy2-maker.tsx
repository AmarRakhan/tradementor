...
export function AsterStrategy2Maker({ snapshot, serverConfirmed, onConfirmed, onChanged }: { snapshot: Record<string, unknown> | null; serverConfirmed: boolean; onConfirmed: (strategy2: Record<string, unknown>) => void; onChanged: () => void }) {
  ...
  useEffect(() => {
    ...
    const persistedFixedPositionSize = String(x.entrySizingMode || "margin").toLowerCase() === "notional";
    // Never manufacture a position-size setting from margin × leverage. Legacy
    // margin-mode entryNotionalUsd values were derived aliases, not user input.
    // Only surface notional values that were stored while notional mode is active
    // (or explicit side-specific values that already exist).
    const storedLongNotional = x.entryNotionalLongUsd ?? x.entryNotionalLong ?? (persistedFixedPositionSize ? x.entryNotionalUsd : undefined);
    ...
    setV({
      ...
      entryNotionalLong: storedLongNotional === undefined || storedLongNotional === null ? "" : txt(storedLongNotional, 0),
      entryNotionalShort: storedShortNotional === undefined || storedShortNotional === null ? "" : txt(storedShortNotional, 0),
  ...
  const settings = (() => {
    ...
    const sizingSettings = v.fixedPositionSize ? {
      entrySizingMode: "notional",
      entryNotionalUsd: longNotional,
      entryNotionalLongUsd: longNotional,
      entryNotionalShortUsd: shortNotional,
      entryNotionalLong: longNotional,
      entryNotionalShort: shortNotional,
    } : {
      // Turning fixed size OFF restores margin execution without rewriting the
      // separately stored notional values. Margin and notional are independent.
    ...
    <div className="maker-input compact-settings-grid">
      ...
      <section className="side-settings-block">
        ...
        {v.tpMode === "PORTFOLIO" && <section className="portfolio-tp-panel" data-visual-reference={PORTFOLIO_TP_REFERENCE} aria-label="Portfolio Take Profit">
          <header className="portfolio-tp-head"><span className="portfolio-tp-icon">◎</span><span><b>Portfolio Take Profit</b><small>Per-trade TP blijft actief. Dit doel sluit daarnaast alle posities zodra de portfoliowaarde is bereikt.</small></span><i title="Portfolio TP werkt aanvullend op de pair-TP, sluit alle posities veilig op het portfoliodoel en start daarna een nieuwe cycle vanaf de werkelijke sluitwaarde.">i</i></header>
        ...
        <div className="side-columns">
          <section className="side-card long"><b>LONG</b><Field label="Instap LONG · margin" value={v.entryMarginLong} set={(value) => change({ ...v, entryMarginLong: value })} suffix="USDT" />{v.fixedPositionSize && <Field label="Vaste positie LONG" value={v.entryNotionalLong} set={(value) => change({ ...v, entryNotionalLong: value })} suffix="USDT" />}<Field label="DCA-afstand LONG" value={v.longDcaDistance} set={(value) => change({ ...v, longDcaDistance: value })} suffix="%" disabled={v.smartRescueEnabled} /><Field label="DCA-bedrag LONG" value={v.longDcaAmount} set={(value) => change({ ...v, longDcaAmount: value })} suffix="USDT" disabled={v.smartRescueEnabled} /><Field label="Max DCA LONG" value={v.maxDcaLong} set={(value) => change({ ...v, maxDcaLong: value })} disabled={v.smartRescueEnabled} /><Field label="Take Profit LONG" value={v.longTp} set={(value) => change({ ...v, longTp: value })} suffix="%" disabled={v.tpMode === "OFF"} /></section>
          <section className={`side-card short ${v.smartRescueEnabled ? "inactive" : ""}`}><b>SHORT {v.smartRescueEnabled ? "· bewaard, runtime uit" : ""}</b><Field label="Instap SHORT · margin" value={v.entryMarginShort} set={(value) => change({ ...v, entryMarginShort: value })} suffix="USDT" disabled={v.smartRescueEnabled} />{v.fixedPositionSize && <Field label="Vaste positie SHORT" value={v.entryNotionalShort} set={(value) => change({ ...v, entryNotionalShort: value })} suffix="USDT" disabled={v.smartRescueEnabled} />}<Field label="DCA-afstand SHORT" value={v.shortDcaDistance} set={(value) => change({ ...v, shortDcaDistance: value })} suffix="%" disabled={v.smartRescueEnabled} /><Field label="DCA-bedrag SHORT" value={v.shortDcaAmount} set={(value) => change({ ...v, shortDcaAmount: value })} suffix="USDT" disabled={v.smartRescueEnabled} /><Field label="Max DCA SHORT" value={v.maxDcaShort} set={(value) => change({ ...v, maxDcaShort: value })} disabled={v.smartRescueEnabled} /><Field label="Take Profit SHORT" value={v.shortTp} set={(value) => change({ ...v, shortTp: value })} suffix="%" disabled={v.smartRescueEnabled || v.tpMode === "OFF"} /></section>
        </div>
      </section>
