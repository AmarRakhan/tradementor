...
export async function GET(request: Request) {
  ...
  try {
    ...
    const tickerBySymbol = new Map(tickers.filter((row): row is Record<string, unknown> => Boolean(row && typeof row === "object"))
    ...
    const markets = await mapLimited(activeSymbols, async (symbol) => {
