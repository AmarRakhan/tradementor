...
def _aster_strategy2_owned_keys(uid:str)->set[tuple[str,str]]:
    ...
    keys={(str(row.get("symbol","")).upper(),str(row.get("side","")).upper())
        for row in rows if str(row.get("symbol","")).strip() and str(row.get("side","")).upper() in {"LONG","SHORT"}}
    # Multi BB is the active Strategy-2 engine, but its durable ownership lives
    # in multiBbPositions instead of legacy ownedLegs. Include only well-formed
    # managed leg keys; the caller still intersects these claims with fresh
    # Aster position_risk(), so stale runtime rows cannot become close orders.
    managed=raw.get("multiBbPositions") if isinstance(raw.get("multiBbPositions"),dict) else {}
    for raw_key,row in managed.items():
        if not isinstance(row,dict) or row.get("botManaged") is False:
            continue
        try:
            symbol,side=str(raw_key).upper().split("|",1)
        except ValueError:
            continue
...
def _portfolio_daily_growth(user:dict[str,Any])->dict[str,Any]:
    """Cashflow-neutral daily performance anchored only to this calendar day."""
    ...
    try:
        ...
        _persist_portfolio_chart_sample(user,equity=equity,source_at_ms=now_ms)
        same_day_candles=_read_portfolio_chart_candles(user,"5m",400)
        anchor=select_day_start_snapshot(
            same_day_candles,day_start_ms=day_start_ms,
            current_equity=equity,current_at_ms=now_ms,
        )
        reference_equity=safe_float(anchor.get("equity"));reference_at_ms=int(safe_float(anchor.get("atMs")))
        if reference_equity<=0 or reference_at_ms<day_start_ms:
            raise ValueError("Dagstart-snapshot is niet betrouwbaar")
        income=client.income_history(start_time=reference_at_ms,limit=1000)
        ...
        def update(txn:Any)->dict[str,Any]:
            ...
            if schema!=PORTFOLIO_DAILY_GROWTH_SCHEMA_VERSION:
                ...
                if bool(state.get("lastObservedReliable")):
                    ...
                    completed_day={"date":str(state.get("lastObservedDate")),
            ...
            completed_sum=safe_float(state.get("completedReturnSum"))
            average_pct=average_daily_return(completed_sum,completed_count,today_pct)
            state.update({"schemaVersion":PORTFOLIO_DAILY_GROWTH_SCHEMA_VERSION,"referenceDate":today,
                "referenceEquity":reference_equity,"referenceAtMs":reference_at_ms,
                "lastObservedDate":today,"lastObservedStartEquity":reference_equity,
                "lastObservedEquity":equity,"lastObservedAtMs":now_ms,
                "lastObservedCashflow":day_net_cashflow,"lastObservedUsd":today_usd,
                "lastObservedReturnPercentage":today_pct,"lastObservedReliable":True,"updatedAt":now})
            txn.set(ref,{"dailyGrowth":state},merge=True)
            history=[row for row in list(state.get("history") or []) if isinstance(row,dict)]
            has_intraday_cashflow=int(cashflow_breakdown.get("count",0))>0
        ...
        return {"reliable":False,"measurementStartDate":local_now.date().isoformat(),
            "performanceMethod":"SAME_DAY_SNAPSHOT_NET_CASHFLOW_ADJUSTED",
...
# Friends is read-only community analytics: no trading or exchange side effects.
