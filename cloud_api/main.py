...
def aster_strategy2_readiness(authorization: str | None = Header(default=None),
                              user: dict[str, Any] = Depends(authenticated_user)) -> dict[str, Any]:
    ...
    for row in raw.get("ownedLegs") if isinstance(raw.get("ownedLegs"),list) else []:
        try:
            leg=owned_from_mapping(row)
            if leg.strategy_id=="aster-strategy-2" and leg.engine_type=="strategy2":owned.append(leg)
        except (TypeError,ValueError): pass
    ...
    active_keys={(str(x.get("symbol","")).upper(),str(x.get("positionSide","")).upper()) for x in positions if abs(safe_float(x.get("positionAmt")))>0}
    source_recovery={"attempted":False,"accepted":False,"strategy2Recovered":0,"strategy3Recovered":0}
    if active_keys-(strategy1_keys|strategy2_keys|strategy3_keys):
        source_recovery["attempted"]=True
        source_status=_read_established_aster_status(authorization)
        evidence=matching_source_ownership(current_positions=positions,source_status=source_status or {},
            config_version=int(safe_float(raw.get("configVersion"))) or 1)
        source_recovery["accepted"]=evidence.accepted
        source_recovery["reason"]=evidence.reason
        source_s2={(x.symbol,x.side) for x in evidence.strategy2_legs}
        source_s3={(x.symbol,x.side) for x in evidence.strategy3_legs}
        ownership_disagrees=bool((strategy2_keys&source_s3)|(strategy3_keys&source_s2))
        if evidence.accepted and not ownership_disagrees:
            now=datetime.now(timezone.utc)
            recovered_s2=[x for x in evidence.strategy2_legs if (x.symbol,x.side) not in strategy2_keys]
            recovered_s3=[x for x in evidence.strategy3_legs if (x.symbol,x.side) not in strategy3_keys]
            if recovered_s2:
                existing_rows=[dict(x) for x in raw.get("ownedLegs",[]) if isinstance(x,dict)]
                aster_strategy2_reference(uid).set({"ownedLegs":[*existing_rows,*[owned_to_mapping(x) for x in recovered_s2]],"updatedAt":now},merge=True)
                for leg in recovered_s2:
                    aster_strategy2_reference(uid).collection("audit").add({"event":"OWNERSHIP_RECOVERED_FROM_ESTABLISHED_STATUS",
                        "strategyId":"aster-strategy-2","symbol":leg.symbol,"side":leg.side,"timestamp":now})
                owned.extend(recovered_s2);strategy2_keys|={(x.symbol,x.side) for x in recovered_s2}
            if recovered_s3:
                existing_s3=[dict(x) for x in strategy3_raw.get("ownedLegs",[]) if isinstance(x,dict)]
                aster_strategy3_reference(uid).set({"ownedLegs":[*existing_s3,*[owned_to_mapping(x) for x in recovered_s3]],"updatedAt":now},merge=True)
                for leg in recovered_s3:
                    aster_strategy3_reference(uid).collection("audit").add({"event":"OWNERSHIP_RECOVERED_FROM_ESTABLISHED_STATUS",
                        "strategyId":"aster-strategy-3","symbol":leg.symbol,"side":leg.side,"timestamp":now})
                strategy3_keys|={(x.symbol,x.side) for x in recovered_s3}
            source_recovery["strategy2Recovered"]=len(recovered_s2)
            source_recovery["strategy3Recovered"]=len(recovered_s3)
        elif ownership_disagrees:
            source_recovery["accepted"]=False
            source_recovery["reason"]="Lokale en gevestigde Strategy-ownership spreken elkaar tegen"
    ...
    report["sourceOwnershipRecovery"]=source_recovery
