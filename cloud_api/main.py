...
def aster_status(user: dict[str, Any] = Depends(authenticated_user)) -> dict[str, Any]:
    ...
    for raw in snapshot.get("positions") if isinstance(snapshot.get("positions"), list) else []:
        ...
        if strategy_id=="aster-strategy-2":
            ...
            if isinstance(focus_slot,dict) and str(row.get("strategy2Role","")).upper().startswith("FOCUS_SLOT:"):
                ...
                next_focus_dca=(anchor*(1-strategy2_settings.focus_dca_distance) if strategy2_settings.focus_dca_enabled and anchor>0 and (strategy2_settings.focus_dca_unlimited or filled<strategy2_settings.focus_max_dca) else 0.0)
                row["dcaCount"]=filled
    ...
    if v2_symbol and str(v2_state.get("cycleId", "")):
        ...
        recovery_model_version=max(1,int(safe_float(v2_state.get("recoveryModelVersion",1))))
        ...
        profit_trigger=(0.0 if recovery_model_version>=3 else strategy2_settings.focus_v2_profit_trigger_usdt);profit_harvest=(0.0 if recovery_model_version>=3 else strategy2_settings.focus_v2_profit_harvest_usdt)
        ...
        target_short_notional=safe_float(v2_history.get("targetShortNotional"))
        ...
        elif profit_trigger>0 and profit_remaining>0: next_action=f"NOG ${profit_remaining:.2f} TOT PROFIT HARVEST"
