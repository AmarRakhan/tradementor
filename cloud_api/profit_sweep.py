...
This module intentionally knows nothing about wallet balance, notional, margin,
principal or unrealized PnL. A contribution can only be derived from positive
net realized profit and the user's configured savings percentage.
"""
...
def sweep_contribution(net_realized_profit: Any, sweep_percent: Any) -> Decimal:
...
def build_sweep_booking(*, closure_id: str, net_realized_profit: Any, sweep_percent: Any) -> dict[str, Any]:
    """Snapshot the percentage used for one immutable close/cycle booking."""
    close_key = str(closure_id or "").strip()
    if not close_key:
        raise ProfitSweepError("closure_id ontbreekt")
    profit = _decimal(net_realized_profit, label="Gerealiseerde nettowinst")
    percent = normalize_sweep_percent(sweep_percent)
    contribution = sweep_contribution(profit, percent)
    return {
        "closureId": close_key,
