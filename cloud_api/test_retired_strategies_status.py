...
def test_strategy2_tick_ignores_retired_strategy_ownership_state():
    source = Path(__file__).with_name("main.py").read_text(encoding="utf-8")
    tick = source[source.index("def _run_aster_strategy2_tick("):source.index("def aster_automation_public(")]
    assert "aster_strategy3_reference(uid)" not in tick
...
def test_production_aster_scheduler_never_runs_retired_strategy1():
    source = Path(__file__).with_name("main.py").read_text(encoding="utf-8")
    scheduler = source[source.index("def run_aster_automation_scheduler("):source.index('@app.post("/internal/aster-strategy3/tick")')]
