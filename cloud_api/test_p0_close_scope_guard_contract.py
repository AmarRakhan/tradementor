...
def test_guard_preserves_registered_production_entrypoint():
    assert "import withdraw_app as _entry" in SOURCE
    assert "app = CloseScopeGuard(_entry.app)" in SOURCE
    assert "uvicorn p0_close_scope_guard:app" in DOCKERFILE


def test_legacy_compatibility_never_mixes_scoped_candidates():
