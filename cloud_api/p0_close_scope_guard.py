...
Every live bulk close must carry one explicit side scope: ALL, LONG or SHORT.
The compatibility filter keeps the currently deployed legacy backend safe until
all production revisions natively accept the ``side`` query parameter.
"""
...
# The live revision that experienced the incident predates native side scoping.
# Patch only its module-level selector. ContextVar isolation keeps concurrent
# requests/background work from inheriting another request's scope.
