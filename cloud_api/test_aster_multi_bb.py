...
def _managed_state_for(position_rows):
    state={}
    for row in position_rows:
        symbol=str(row["symbol"]).upper()
        side=str(row["positionSide"]).upper()
        state[f"{symbol}|{side}"]={
            "cycleId":f"managed-{symbol}-{side}",
            "dcaCount":0,
            "lastBotFillPrice":float(row.get("entryPrice",100)),
            "lastKnownQty":abs(float(row.get("positionAmt",1))),
            "lastKnownEntry":float(row.get("entryPrice",100)),
            "leverage":int(float(row.get("leverage",100))),
            "cycleStartedAtMs":1,
            "updatedAtMs":1,
            "botManaged":True,
        }
...
def test_exchange_short_count_at_configured_cap_blocks_new_short_even_when_strategy_ownership_undercounts():
    shorts=[
        {"symbol":f"S{i}USDT","positionSide":"SHORT","positionAmt":"1","entryPrice":"100","markPrice":"100","leverage":"100"}
        for i in range(7)
    ]
    managed=shorts[:2]
    tickers=[{"symbol":f"N{i}USDT","quoteVolume":str(1000-i)} for i in range(14)]
    prices={**{row["symbol"]:100 for row in shorts},**{row["symbol"]:100 for row in tickers}}
    client=Client(positions=shorts,tickers=tickers,prices=prices,leverage=100)
    settings=cfg(universeTopN=14,maximumPositions=14,longSlots=7,shortSlots=7,entrySizingMode="margin",entryMarginUsd=.3)
    result=run_multi_bb_step(client=client,ref=Ref(),raw_state=_managed_state_for(managed),settings=settings,uid="u",
        account={"availableBalance":"100"},positions=shorts,open_orders=[],timestamp_ms=int(time.time()*1000),dry_run=True,order_budget=14)

    entries=[row for row in result["actions"] if row["kind"]=="ENTRY"]
    assert entries
    assert all(row["side"]=="LONG" for row in entries)
    assert result["exchangeActiveShort"]==7
    assert result["shortSideHardCapReached"] is True
    assert result["remainingShort"]==0


def test_exchange_short_count_above_configured_cap_never_allocates_another_short():
    shorts=[
        {"symbol":f"S{i}USDT","positionSide":"SHORT","positionAmt":"1","entryPrice":"100","markPrice":"100","leverage":"100"}
        for i in range(12)
    ]
    ...
    assert not any(row["kind"]=="ENTRY" and row["side"]=="SHORT" for row in result["actions"])
    assert result["exchangeActiveShort"]==12
    assert result["shortSideHardCapReached"] is True
    assert result["remainingShort"]==0


def test_exchange_long_count_at_configured_cap_blocks_new_long_symmetrically():
    longs=[
        {"symbol":f"L{i}USDT","positionSide":"LONG","positionAmt":"1","entryPrice":"100","markPrice":"100","leverage":"100"}
        for i in range(7)
    ]
    managed=longs[:2]
    tickers=[{"symbol":f"N{i}USDT","quoteVolume":str(1000-i)} for i in range(14)]
    prices={**{row["symbol"]:100 for row in longs},**{row["symbol"]:100 for row in tickers}}
    client=Client(positions=longs,tickers=tickers,prices=prices,leverage=100)
    settings=cfg(universeTopN=14,maximumPositions=14,longSlots=7,shortSlots=7,entrySizingMode="margin",entryMarginUsd=.3)
    result=run_multi_bb_step(client=client,ref=Ref(),raw_state=_managed_state_for(managed),settings=settings,uid="u",
        account={"availableBalance":"100"},positions=longs,open_orders=[],timestamp_ms=int(time.time()*1000),dry_run=True,order_budget=14)

    entries=[row for row in result["actions"] if row["kind"]=="ENTRY"]
    assert entries
    assert all(row["side"]=="SHORT" for row in entries)
    assert result["exchangeActiveLong"]==7
    assert result["longSideHardCapReached"] is True
