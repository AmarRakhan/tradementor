(() => {
  ...
  async function copyDynamics(){
    ...
    const hasSerial=rows.some(r=>r.serial);
    const txt=rows.map(r=>{
      if(!hasSerial) return `${r.code}\t${r.status}`;

      // Dynamics plakt alleen naar bewerkbare gridvelden.
      // Productnaam is read-only en wordt tijdens plakken overgeslagen.
      // Bewerkbare volgorde vanaf Artikelnummer:
      // 1 Artikelnummer
      // 2 Voorraadstatus
      // 3 Voorraadstatus
      // 4 Hoeveelheid
      // 5 Eenheid
      // 6 Serienummer
      return [r.code,r.status,'','','',r.serial||''].join('\t');
    }).join('\n');
    ...
    toast(hasSerial
      ? `${rows.length} regels gekopieerd · serienummer naar Dynamics`
