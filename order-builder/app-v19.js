(() => {
  ...
  function isNoiseTicketLine(line){
    ...
    // Alleen pure serienummerregels zonder herkenbare producttekst worden later genegeerd.
  ...
  function analyzeTicketText(text){
    ...
    return parseTicketLines(text)
      ...
      const suggestions=fixed
        ? [{item:fixed,confidence:99,reason:'vaste bedrijfsregel'}]
        : rankSmartMatches(line.query);
      const first=suggestions[0]||null;
      const second=suggestions[1]||null;

      let selectedCode=fixed ? fixed.code : null;
      let auto=!!fixed;
  ...
  function previewTicket(){
    ...
    renderTicketPreview(window.__ticketResults);
    (window.__ticketPackageIntents||[]).slice().reverse().forEach(intent=>{
